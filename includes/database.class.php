...
$domainame = SERVER_PROTOCOL.$_SERVER['SERVER_NAME']."/";
...
class MysqliDatabase extends mysqli
{
	...
	*/
	public function __construct($ConnectionString, $ThrowExceptions = true)
	{
		...
		@$this->connect(empty($CS['server']) ? DB_HOST : $CS['server'],
							 empty($CS['username']) ? DB_USERNAME : $CS['username'],
							 empty($CS['password']) ? DB_PASSWORD : $CS['password'],
							 empty($CS['database']) ? DB_DATABASE : $CS['database'],
