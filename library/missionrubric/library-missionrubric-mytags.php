...
$expeditionname = $ObjDB->SelectSingleValue("SELECT CONCAT(a.fld_mis_name,' ',b.fld_version)
...
$rubricname=$ObjDB->SelectSingleValue("SELECT fn_shortname (CONCAT(fld_rub_name), 1) FROM itc_mis_rubric_name_master WHERE fld_mis_id='".$missionid."' AND fld_id='".$rubricid."' AND fld_delstatus='0'");
...
<section data-type='2home' id='library-missionrubric-mytags'>
    <div class='container'>
        ...
        <script type="text/javascript" charset="utf-8">
           ...
           $.getScript("library/missionrubric/library-missionrubric-mytags.js");	
        </script> 
        ...
        <div class='row '>
            <div class='twelve columns centered insideForm'>
                <form name="exporderforms" id="exporderforms">
                        ...
                        $qrydest=$ObjDB->QueryObject("SELECT a.fld_id as destid,a.fld_dest_name as destname 
															...
															WHERE a.fld_mis_id='".$missionid."' AND a.fld_rubric_name_id='".$rubricid."'
                        ...
                        for($i=0;$i<sizeof($did);$i++) 
                        {
                            ...
                            $qryviewexp_rubric=$ObjDB->QueryObject("SELECT c.fld_id as rowid, a.fld_id as destid, c.fld_category as category FROM itc_mis_rubric_dest_master as a
																	LEFT JOIN itc_mis_rubric_master AS c ON c.fld_destination_id=a.fld_id 
																	WHERE c.fld_rubric_id='".$rubricid."' AND c.fld_mis_id='".$missionid."' 
																	AND c.fld_destination_id='".$did[$i]."' AND a.fld_delstatus='0' AND c.fld_delstatus='0' ");//AND c.fld_created_by='".$uid."'
                    ...
                    <!-- Autocomplete script start -->
                            ...
                            <script type="text/javascript" charset="utf-8">	
                                $(function(){				
                                    ...
                                    ?>	
                                    var expid='<?php echo $missionid; ?>';
                                    var destid='<?php echo $rowid; ?>';
                                    t5.getContainer().addClass('textboxlist-loading');				
                                    $.ajax({url: 'autocomplete.php', data: 'oper=search&tag_type=39&expid='+expid+'&rowid='+destid, type:"POST", dataType: 'json', success: function(r){
            ...
            <div class="row rowspacer" style="margin-top:20px;">
                ...
                <div class="tRight">
                    <input type="button" class="darkButton" style="width:200px; height:42px;float:right;margin-bottom:20px;margin-right:20px;" value="Save Status" onClick="fn_savecontenttagstatus(<?php echo $missionid; ?>,<?php echo $rubricid; ?>);" />
                </div>
            </div>
        </div>
