                                   ...
                                   <ul role="options" style="width:100%">
                                       <?php
                                        $qry = $ObjDB->QueryObject("SELECT w.* FROM (
                                                                        (SELECT a.fld_class_id AS classid, a.fld_schedule_name AS schedulename,18 AS exptype,
                                                                        a.fld_id AS assingnmentid FROM itc_class_indasmission_master AS a
                                                                        LEFT JOIN itc_class_master AS b ON b.fld_id=a.fld_class_id
                                                                        WHERE a.fld_delstatus='0' AND a.fld_flag='1' AND a.fld_createdby='".$uid."' 
                                                                        AND b.fld_delstatus = '0' AND b.fld_flag = '1')
                                                                    UNION ALL
                                                                        (SELECT a.fld_class_id AS classid, a.fld_schedule_name AS schedulename,19 AS exptype,
                                                                        a.fld_id AS assingnmentid FROM itc_class_rotation_mission_mastertemp AS a
                                                                        LEFT JOIN itc_class_master AS b ON b.fld_id=a.fld_class_id
                                                                        WHERE a.fld_delstatus='0' AND a.fld_flag='1' AND a.fld_createdby='".$uid."'
                                                                        AND b.fld_delstatus = '0'  AND b.fld_flag = '1'))AS w  ORDER BY w.schedulename"); //group by w.assingnmentid
                                        if($qry->num_rows>0){
                                            while($row = $qry->fetch_assoc())
                                            {
                                                extract($row);
