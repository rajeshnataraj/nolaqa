...
{
	error_reporting(0);
	$mis_id= isset($method['exp_id']) ? $method['exp_id'] : '';
        ...
        <form name="expdestform" id="expdestform" >
           ...
           <div class="row" style="min-width:400px;">  
               ...
               <div class="four columns">
                   ...
                   <dl class="field row">
                      <dt class="text">
                      <textarea placeholder='4' id="txtdest4" name="txtdest4"   tabindex="2" ><?php echo htmlentities($four); ?></textarea>
                      </dt> 
                   </dl>
               </div>
               <div class="four columns">
                   ...
                   <dl class="field row">
                      <dt class="text">
                      <textarea placeholder='3' id="txtdest3" name="txtdest3"  tabindex="3" ><?php echo htmlentities($three);  ?></textarea>
                      </dt> 
                   </dl>
               </div>
           </div> 
        <div class="row" style="min-width:400px;">  
            ...
            <div class="row rowspacer" style="min-width:400px;">
               <div style="margin-left:<?php if($type==1){echo '129px'; }else {echo '135px';} ?>;margin-right:10px;" >
                    <?php if($type==1){ ?>
                         <input style="margin-right:10px;width:90px" onclick="fn_tempsavedestexpform1('<?php echo ($mis_id); ?>','<?php echo $dest_id; ?>','<?php echo $rowid; ?>','<?php echo $type; ?>','<?php echo $rubid; ?>','<?php echo $uid; ?>','<?php echo $rcount; ?>','<?php echo $empdestid; ?>','<?php echo $score; ?>','<?php echo $qryforgetdestdetails; ?>');" type="button" class="module-extend-button" id="saveextend_btn" value="Ok" /> 
                         <input style="margin-right:10px;width:90px" onclick="fn_deletedestexpform('<?php echo ($mis_id); ?>','<?php echo $dest_id; ?>','<?php echo $rowid; ?>','<?php echo $type; ?>','<?php echo $rubid; ?>','<?php echo $uid; ?>','<?php echo $score; ?>');" type="button" class="module-extend-button" id="saveextend_btn" value="Delete" /> 
                    <?php   } 
                    elseif($type==2){ ?>
                         <input style="margin-right:10px;width:90px" onclick="fn_tempsavedestexpform1('<?php echo ($mis_id); ?>','<?php echo $dest_id; ?>','<?php echo $rowid; ?>','<?php echo $type; ?>','<?php echo $rubid; ?>','<?php echo $uid; ?>','<?php echo $rcount; ?>','<?php echo $empdestid; ?>','<?php echo $score; ?>','<?php echo $qryforgetdestdetails; ?>');" type="button" class="module-extend-button" id="saveextend_btn" value="Ok" /> 
                         <input style="margin-right:10px;width:90px" onclick="fn_deletedestexpform('<?php echo ($mis_id); ?>','<?php echo $dest_id; ?>','<?php echo $rowid; ?>','<?php echo $type; ?>','<?php echo $rubid; ?>','<?php echo $uid; ?>','<?php echo $score; ?>');" type="button" class="module-extend-button" id="saveextend_btn" value="Delete" /> 
                    ...
                    <?php   }
                        ...
                        $startval=$ObjDB->SelectSingleValueInt("SELECT fld_dest_id FROM itc_mis_rubric_master WHERE fld_mis_id='".$mis_id."' and fld_delstatus='0' and fld_rubric_id='".$rubid."'");
                        
                        if($startval == '')
						{
                            $startval=$ObjDB->SelectSingleValueInt("SELECT fld_id FROM itc_mis_destination_master WHERE fld_mis_id='".$mis_id."' and fld_delstatus='0' ");
					 	}
						else
						{
							$startval=0;
                        ...
                        $destcountempty=$ObjDB->SelectSingleValueInt("SELECT count(fld_dest_id) FROM itc_mis_rubric_master WHERE fld_mis_id='".$mis_id."' AND fld_dest_id='".$dest_id."' and fld_delstatus='0' and fld_rubric_id='".$rubid."'");
                        if($destcountempty=='0'){
                             ?>
                        ...
                        $countdest=$ObjDB->QueryObject("SELECT fld_dest_id as destinationid FROM itc_mis_rubric_master WHERE fld_dest_id between $startval and $dest_id and fld_mis_id='".$mis_id."' and fld_delstatus='0' and fld_rubric_id='".$rubid."' group by fld_dest_id");
                        ...
                        ?>
                         <input style="margin-right:10px;width:90px" onclick="fn_tempsavedestexpform('<?php echo ($mis_id); ?>','<?php echo $dest_id; ?>','<?php echo $rowid; ?>','<?php echo $type; ?>','<?php echo $rubid; ?>','<?php echo $uid; ?>','<?php echo $empdestid; ?>','<?php echo $score; ?>','<?php echo $qryforgetdestdetails; ?>');" type="button" class="module-extend-button" id="saveextend_btn" value="Ok" /> 
...
if($oper=="deletedestexpform" and $oper != " " )
{
    try
    {
		$misid= isset($method['exp_id']) ? $method['exp_id'] : '';
...
if($oper=="saveasrubric")
{
    try
	{
		$misid = isset($method['expid']) ? $method['expid'] : '0'; 
        ...
        if($updateid=='0')
        {
            $maxid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_mis_rubric_name_master (fld_rub_name, fld_mis_id, fld_created_by, fld_created_date, fld_district_id, fld_school_id, fld_user_id,fld_profile_id) 
                                            VALUES ('".$rubname."', '".$misid."', '".$uid."', '".$date."','".$sendistid."','".$schoolid."','".$indid."','".$sessmasterprfid."')");

        }
        else
        {
            $ObjDB->NonQuery("UPDATE itc_mis_rubric_name_master SET fld_rub_name='".$ownnrubname."', fld_updated_by='".$uid."', 
									 fld_updated_date='".$date."' WHERE fld_mis_id='".$misid."' AND fld_id='".$rubricnameid."'");
			
			$ObjDB->NonQuery("UPDATE itc_mis_rubric_dest_master 
								 SET fld_delstatus='1', fld_deleted_by='".$uid."', fld_deleted_date='".date("Y-m-d H:i:s")."'
								 WHERE fld_rubric_name_id='".$rubricnameid."' AND fld_mis_id='".$misid."'");
			
            $ObjDB->NonQuery("UPDATE itc_mis_rubric_master
									 SET fld_delstatus='1', fld_deleted_by='".$uid."', fld_deleted_date='".date("Y-m-d H:i:s")."'
									 WHERE fld_rubric_id='".$rubricnameid."' AND fld_mis_id='".$misid."'");
	...
	/*************Existing rubric statement start***********/
        for($i=0;$i<sizeof($rubnametemp);$i++) 
        {
            ...
            if($destid!='')
            {
				...
				{
					$destcount = $ObjDB->SelectSingleValueInt("SELECT fld_id FROM itc_mis_rubric_dest_master 
														WHERE fld_rubric_name_id='".$maxid."' AND fld_mis_id='".$misid."' 
														AND fld_dest_name='".$ObjDB->EscapeStr($destname)."' ");
					if($destcount==0)
					{
						$newrowdestid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_mis_rubric_dest_master (fld_rubric_name_id, fld_mis_id, fld_dest_name, fld_created_by, fld_created_date) 
								VALUES ('".$maxid."', '".$misid."', '".$ObjDB->EscapeStr($destname)."', '".$uid."', '".date("Y-m-d H:i:s")."')");

					}
					else
					{
						$ObjDB->NonQuery("UPDATE itc_mis_rubric_dest_master SET fld_dest_name='".$ObjDB->EscapeStr($destname)."',
										fld_updated_by='".$uid."', fld_updated_date='".date("Y-m-d H:i:s")."',fld_delstatus='0' 
									WHERE fld_rubric_name_id='".$maxid."' AND fld_mis_id='".$misid."' AND fld_id='".$destcount."' ");//update already exist
					...
					if($updateid=='1')
					{
						...
						{
							$destcount = $ObjDB->SelectSingleValueInt("SELECT fld_id FROM itc_mis_rubric_master 
																		WHERE fld_rubric_id='".$maxid."' AND fld_mis_id='".$expid."' 
							...
							if($destcount==0)
							{
								...
								$newrowid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_mis_rubric_master (`fld_rubric_id`,`fld_mis_id`, `fld_destination_id`, `fld_category`, `fld_four`, `fld_three`, `fld_two`, `fld_one`, `fld_zer`, `fld_weight`, `fld_score`, `fld_created_by`, `fld_created_date`,`newid`) 
												VALUES ('".$maxid."', '".$misid."', '".$newrowdestid."', '".$category."', '".$four."', '".$three."', '".$two."', '".$one."', '".$zer."', '".$weight."', '".$score."', '".$uid."', '".date("Y-m-d H:i:s")."', '".$newid."')");//insert if not exist 
							}
							else
							{
								$ObjDB->NonQuery("UPDATE itc_mis_rubric_master 
										SET fld_category='".$category."', fld_four='".$four."', fld_three='".$four."', fld_two='".$two."',
										fld_one='".$one."', fld_zer='".$zer."', fld_weight='".$weight."', fld_score='".$score."',
										fld_updated_by='".$uid."', fld_updated_date='".date("Y-m-d H:i:s")."', fld_delstatus='0' 
										WHERE fld_id='".$rubrowid."' AND fld_mis_id='".$misid."' AND fld_destination_id='".$newrowdestid."' ");//update already exist
							}
						}
					}
					else
					{
						if($category!="undefined")
						{
							...
							$tagcount = $ObjDB->SelectSingleValueInt("SELECT count(fld_id) FROM itc_main_tag_mapping WHERE fld_item_id='".$rubrowid."' AND fld_tag_type='39' AND fld_access='1'");
							if($tagcount !='0')
							{
								$qrytag = $ObjDB->QueryObject("SELECT a.fld_id AS tagid
																				  FROM itc_main_tag_master AS a, itc_main_tag_mapping AS b 
																											  WHERE a.fld_id=b.fld_tag_id AND b.fld_tag_type='39' 
																													AND b.fld_access='1' 
								...
								if($qrytag->num_rows > 0)
								{
									while($row=$qrytag->fetch_assoc())
									{
										...
										$ObjDB->NonQuery("INSERT INTO itc_main_tag_mapping (fld_tag_id,fld_tag_type,fld_item_id,fld_access,fld_lesson_flag) 
															VALUES ('".$tagid."','39','".$newrowid."','1','0')");//insert if not exist
				...
				if($destname!="undefined")
				{
					...
					if($updateid=='1')
					{
					   ...
					   $ObjDB->NonQuery("INSERT INTO itc_mis_rubric_master (`fld_rubric_id`,`fld_mis_id`, `fld_destination_id`, `fld_category`, `fld_four`, `fld_three`, `fld_two`, `fld_one`, `fld_zer`, `fld_weight`, `fld_score`, `fld_created_by`, `fld_created_date`,`newid`) 
									   VALUES ('".$rubricnameid."', '".$misid."', '".$newrowdestid."', '".$category1."', '".$four1."', '".$three1."', '".$two1."', '".$one1."', '".$zer1."', '".$weight1."', '".$score1."', '".$uid."', '".date("Y-m-d H:i:s")."', '".$newid."')");//insert if not exist
					...
					else
					{
						...
						$newid=$newid+1;
						$ObjDB->NonQuery("INSERT INTO itc_mis_rubric_master (`fld_rubric_id`,`fld_mis_id`, `fld_destination_id`, `fld_category`, `fld_four`, `fld_three`, `fld_two`, `fld_one`, `fld_zer`, `fld_weight`, `fld_score`, `fld_created_by`, `fld_created_date`,`newid`) 
										VALUES ('".$maxid."', '".$misid."', '".$newrowdestid."', '".$category1."', '".$four1."', '".$three1."', '".$two1."', '".$one1."', '".$zer1."', '".$weight1."', '".$score1."', '".$uid."', '".date("Y-m-d H:i:s")."', '".$newid."')");//insert if not exist
...
if($oper=="checkrubricname")
{
    ...
    $misid = isset($method['uid']) ? $method['uid'] : '0'; 
...
if($oper=="deleterubric" and $oper != " " )
{
    try
    {
        $misid= isset($method['exp_id']) ? $method['exp_id'] : '0';
        $rowid= isset($method['rowid']) ? $method['rowid'] : '0';
        ...
        if($validate_assetid)
                $ObjDB->NonQuery("UPDATE itc_mis_rubric_name_master
                                         SET fld_delstatus='1', fld_deleted_by='".$uid."', fld_deleted_date='".date("Y-m-d H:i:s")."'
                                         WHERE fld_id='".$rowid."' AND fld_mis_id='".$misid."'");
        else
                echo "fail";	
    }
    catch(Exception $e)
...
if($oper=="download")
{
	for ($x = 0; $x <= 100000000; $x++) // for time delay
	{
