...
{   
    ...
    { 
		...
		var txtdestscore=$('#txtdestscore').val();	
                ...
                var txtdestcategoryname=txtdestcategoryname.replace(/•/g, "<br />•").replace(/♦/g, "<br />♦").replace(/◘/g, "<br />◘");
                var txtdest4=txtdest4.replace(/•/g, "<br />•").replace(/♦/g, "<br />♦").replace(/◘/g, "<br />◘");
                var txtdest3 = txtdest3.replace(/•/g, "<br />•").replace(/♦/g, "<br />♦").replace(/◘/g, "<br />◘");
                var txtdest2=txtdest2.replace(/•/g, "<br />•").replace(/♦/g, "<br />♦").replace(/◘/g, "<br />◘");
                var txtdest1=txtdest1.replace(/•/g, "<br />•").replace(/♦/g, "<br />♦").replace(/◘/g, "<br />◘");
                var txtdest0=txtdest0.replace(/•/g, "<br />•").replace(/♦/g, "<br />♦").replace(/◘/g, "<br />◘");
        ...
        if(mdestcount==1)
		{
            ...
            var type=3;            
            var Content1='<tr class="exp-rubric-'+destid+'" id="exp-rubric-'+rowCount+'" onclick="fn_showdeststmteditform(\''+id+'\',\''+destid+'\',\''+rowCount+'\',\''+type+'\',\''+rubid+'\',\''+3+'\',\''+destname+'\')">\n\
        ...
        else
		{
            ...
            txtdestweight='X'+txtdestweight;
            var Content1='<tr class="exp-rubric-'+destid+'" id="exp-rubric-'+rowCount+'" onclick="fn_showdeststmteditform(\''+id+'\',\''+destid+'\',\''+rowCount+'\',\''+type+'\',\''+rubid+'\',\''+2+'\',\''+destname+'\')">\n\
...
/****** Used to append the popup form data in to form edit data start here *******/
		   ...
		   url: 'library/rubric/library-rubric-graderubricajax.php',
			...
			{				
				if(data=="success") //Works if the data saved in db
				{
					...
					setTimeout('removesections("#library-rubric");',1000);
					setTimeout('showpageswithpostmethod("library-rubric-rublist","library/rubric/library-rubric-rublist.php","id='+id+'");',1000);
				}
				else
				{
				...
				$.ajax({
						...
						success: function (data) {	
							if(data=="success")
							{
								...
								setTimeout('closeloadingalert()',500);
								setTimeout('removesections("#library-rubric");',1000);
								setTimeout('showpageswithpostmethod("library-rubric-rublist","library/rubric/library-rubric-rublist.php","id='+expid+'");',1000);
							}
							else
							{
								closeloadingalert();	
    ...
    if($("#expdestsecform").validate().form())
    {
		...
		else
		{
			...
			var Content3='<tr value="'+countdest+'" id="countrow-'+countdest+'">\n\
							<td colspan="8"><span onclick="fn_showdeststmtform(\''+expid+'\',\''+tempdestid+'\',\''+0+'\',\''+rubid+'\',\''+countdest+'\',\''+txtdestcategoryname+'\')"><span class="icon-synergy-add-dark small-icon" style="padding-top:10px;"></span> Add additional category to this destination</span></td>\n\
						  </tr></tbody>';
	...
	$.ajax({
		...
		{	
			if(data=="success") //Works if the data saved in db
			{
				...
				var downloadid=1;
				var oper="downloadpdf";				
				setTimeout('showpageswithpostmethod("reports-pdfviewer","reports/reports-pdfviewer.php","id='+val+'&oper='+oper+'&filename='+filename+'&downloadid='+downloadid+'");',500);
