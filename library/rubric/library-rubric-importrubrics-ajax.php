 ...
 <script type="text/javascript" language="javascript">
    ...
    $("#form1").validate({
            ...
            rules: {
                    ...
                    remote:{ 
                                url: "library/rubric/library-rubric-importrubrics-ajax.php", 
                                type:"POST", 
                                data: {  
                                                id: function() {
...
if($oper == "importstudents" and $oper != '') 
{
	...
	$path =(isset( $method['path'])) ?  $method['path'] : '';
        $expid= isset($method['exp']) ? $method['exp'] : '';	
        ...
        { //rubric
           ...
           for($i=2;$i<=sizeof($sheetData);$i++)
           {
               ...
               if($dname != "" and $category != "" and $four != "" and $three != "" and $two != "" and $one != "" and $zero != "" and $weight != "")//and $destidcount == 1
               {
                    ...
                    //new Line Start
                    $countrub = $ObjDB->SelectSingleValueInt("SELECT fld_id FROM itc_exp_rubric_name_master 
                                                      WHERE fld_rub_name='".$rubname."' AND fld_exp_id='".$expid."' and fld_delstatus='0' AND fld_created_by='".$uid."'");
                    if($countrub == 0)
                    { 
                        $rubrid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_exp_rubric_name_master (fld_rub_name, fld_exp_id, fld_created_by, fld_created_date, fld_district_id, fld_school_id, fld_user_id,fld_profile_id) 
                                                        VALUES ('".$rubname."', '".$expid."', '".$uid."', '".$date."','".$sendistid."','".$schoolid."','".$indid."','".$sessmasterprfid."')");
                    }
                    else
                    { 
                        $ObjDB->NonQuery("UPDATE itc_exp_rubric_name_master SET fld_rub_name='".$rubname."', fld_updated_by='".$uid."', 
                                                                         fld_updated_date='".$date."' WHERE fld_exp_id='".$expid."' AND fld_created_by='".$uid."' AND fld_id='".$countrub."' AND fld_delstatus='0'");
                        $rubrid = $countrub;
                    } 
				  $destidcount=$ObjDB->SelectSingleValueInt("SELECT fld_id FROM itc_exp_rubric_dest_master WHERE fld_exp_id='".$expid."' 
												AND fld_dest_name='".$dname."' AND fld_rubric_name_id='".$rubrid."' AND fld_delstatus='0'" );
               
				   if($destidcount=='' || $destidcount=='0' )
				   {
                      $maxid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_exp_rubric_dest_master (fld_rubric_name_id, fld_exp_id, fld_dest_name, fld_created_by, fld_created_date) 
                                                        VALUES ('".$rubrid."', '".$expid."', '".$dname."', '".$uid."','".$date."')");						
				   ...
				   else
				   {
                    ...
                    $ObjDB->NonQuery("INSERT INTO itc_exp_rubric_master (`fld_rubric_id`,`fld_exp_id`, `fld_destination_id`, `fld_category`, `fld_four`, `fld_three`, `fld_two`, `fld_one`, `fld_zer`, `fld_weight`, `fld_score`, `fld_created_by`, `fld_created_date`,`newid`) 
                                         VALUES ('".$rubrid."', '".$expid."', '".$maxid."', '".$category."', '".$four."', '".$three."', '".$two."', '".$one."', '".$zero."', '".$weight."', '".$txtdestscore."', '".$uid."', '".date("Y-m-d H:i:s")."', '".$newid."')");//insert if not exist
...
if($oper=="checkrubricname") //rubric
{
            ...
            $expid = isset($method['id']) ? $method['id'] : '0'; 
            ...
            $count = $ObjDB->SelectSingleValueInt("SELECT count(fld_id) FROM itc_exp_rubric_name_master 
		                                      WHERE MD5(LCASE(REPLACE(fld_rub_name,' ','')))='".$txtrubricname."' 
											   AND fld_delstatus='0' AND fld_exp_id<>'".$expid."'");
