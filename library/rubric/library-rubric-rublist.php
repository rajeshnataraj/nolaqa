...
$expid = isset($method['id']) ? $method['id'] : '';
$expeditionqry = $ObjDB->QueryObject("SELECT fld_exp_name, fld_ui_id
                                    FROM itc_exp_master 
                                    WHERE fld_id='$expid' AND fld_delstatus='0'");
...
<section data-type='2home' id='library-rubric-rublist'>
    ...
    <div class='container'>
        ...
        <div class='row buttons rowspacer' id="rubriclist">
            ...
            <a class='skip btn mainBtn' href='#library-rubric-newgraderubric' id='btnlibrary-rubric-newgraderubric'  name='<?php echo $expid.",".'0';?>'>
          ...
          <?php }?>  
            ...
            if($sessmasterprfid == 2 || $sessmasterprfid == 3 ){ //For Pitsco & Content Admin
                    ...
                    $qry = "SELECT fld_rub_name,fld_id,fld_created_by, fn_shortname (CONCAT(fld_rub_name), 1) AS shortname  FROM itc_exp_rubric_name_master WHERE fld_exp_id='".$expid."' and fld_delstatus='0' AND fld_created_by IN(".$pitscoadmins.")";
            ...
            $qry_for_get_all_expedition = $ObjDB->QueryObject($qry);
                ...
                $pitscoorteacher=$ObjDB->SelectSingleValueInt("SELECT count(fld_id) FROM itc_exp_rubric_name_master WHERE fld_exp_id='".$expid."' AND fld_delstatus='0' AND fld_id='".$fld_id."' AND fld_created_by IN(".$pitscoadmins.")");
                    
                    $profile_id=$ObjDB->SelectSingleValueInt("SELECT fld_profile_id FROM itc_exp_rubric_name_master where fld_id='".$fld_id."' AND fld_delstatus='0' ");
                
                    if($profile_id == 2 || $profile_id == 3 ){ //For Pitsco & Content Admin
                            ?>
                    <a class='skip btn mainBtn <?php if($pitscoorteacher==1){echo "pit"; } ?>' href='#library-rubric' id='btnlibrary-rubric-actions' name="<?php echo $expid.",".$fld_id;?>"><!-- name='<?php //echo $gradestu.",".$id[0];?>'-->
                        ...
                        if($profile_id == 6){ //For District Admin
                            ?>
                            <a class='skip btn mainBtn dis' href='#library-rubric' id='btnlibrary-rubric-actions' name="<?php echo $expid.",".$fld_id;?>"><!-- name='<?php //echo $gradestu.",".$id[0];?>'-->
                                <div class='icon-synergy-modules'></div>
                                <div  class='onBtn tooltip' title="<?php echo $fld_rub_name; ?>"><?php echo $shortname; ?></div>
                            </a>
                            <?php 
                            ...
                            ?>
                            <a class='skip btn mainBtn sch' href='#library-rubric' id='btnlibrary-rubric-actions' name="<?php echo $expid.",".$fld_id;?>"><!-- name='<?php //echo $gradestu.",".$id[0];?>'-->
                                <div class='icon-synergy-modules'></div>
                                <div  class='onBtn tooltip' title="<?php echo $fld_rub_name; ?>"><?php echo $shortname; ?></div>
                            </a>
                            <?php 
