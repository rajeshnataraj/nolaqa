...
<section data-type='2home' id='library-missionmaterials'>
	<!--Script for Tag Well-->
	<script type="text/javascript" charset="utf-8">	
		$.getScript('library/missionmaterials/library-missionmaterials.js');
               			
        $(function(){				
            var t4 = new $.TextboxList('#form_tags_missionmaterials', {
                ...
                plugins: {
                    autocomplete: {
                        onlyFromValues:true,
                        queryRemote: true,
                        remote: { url: 'autocomplete.php', extraParams: "oper=search&tag_type=34&missionmaterial=1" },
                        placeholder: ''
                    }
                },
        ...
        function fn_loadmaterial(){
           var sid = $('#form_tags_missionmaterials').val();
           $("#materialslist").load("library/missionmaterials/library-missionmaterials.php #materialslist > *",{'sid':sid});
           removesections('#library-missionmaterials');
        }
    </script>
    ...
    <div class='container'>
        ...
        <div class='row'>
            <div class='twelve columns'>
                ...
                <div class="tag_well">
                	<input type="text" name="form_tags_missionmaterials" value="" id="form_tags_missionmaterials" />
              	</div>
            </div>
        </div>
        ...
        <div class='row buttons rowspacer' id="materialslist">
            ...
            <a class='skip btn mainBtn' href='#library-missionmaterials' id='btnlibrary-missionmaterials-newmaterial'>
            ...
            if($sessmasterprfid == 2 || $sessmasterprfid == 3)
            {
                  $qry = "SELECT U.fld_id AS materialid, U.fld_materials AS materialname, fn_shortname (U.fld_materials, 1) AS shortname, U.fld_thumbimg_url AS thumbimgicon, fld_upload_path AS uploadmaterialicon
                              FROM itc_mis_materials_master AS U WHERE U.fld_delstatus = '0'  AND U.fld_created_by = '".$uid."' ".$sqry." GROUP BY U.fld_id";
            }
            else {

                  $qry = "SELECT U.fld_id AS materialid, U.fld_materials AS materialname, fn_shortname (U.fld_materials, 1) AS shortname, U.fld_thumbimg_url AS thumbimgicon, fld_upload_path AS uploadmaterialicon
                              FROM itc_mis_materials_master AS U, itc_license_track AS V WHERE U.fld_delstatus = '0' AND U.fld_created_by = '".$uid."' AND V.fld_school_id='".$schoolid."' AND V.fld_user_id=0 ".$sqry." GROUP BY U.fld_id";
            }
            ...
            if($qry_listallmaterials->num_rows>0)
            {
                ...
                extract($res);
                    ?>
                    <a class='skip btn mainBtn' href='#library-materials' id='btnlibrary-missionmaterials-actions' name='<?php echo $materialid.",".$materialname;?>'>
