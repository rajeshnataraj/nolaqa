/*
	Created By - Mohan. M
	Page - library-missionmaterials.js
	History:
*/
/*----
    fn_creatematerials()
...
function fn_creatematerials(id,usersessid)
{
	 if($("#exp_materialform1").validate().form())
        ...
        if(thumbimgurl=='' && uploadimg=='' && catalogurl=='')
            ...
            var dataparam = "oper=savematerials"+"&materialid="+id+"&usersessnid="+usersessid+"&material_name="+escapestr($('#materialname').val())+"&thumb_imageurl="+thumb_url+"&catalogurl="+catalog_url+"&uploadimage="+uploadimg+"&material_desc="+escapestr($('#materialdescription').val())+"&tags="+escapestr($('#form_tags_mewmaterial').val());
            $.ajax({
                    url: "library/missionmaterials/library-missionmaterials-ajax.php",
                    data: dataparam,
                    type: "POST",
                    beforeSend: function(){
                    ...
                    success: function (data) {	
                            ...
                            if(data=="success") //Works if the data saved in db
                            {
                                    ...
                                    setTimeout('showpages("library-missionmaterials","library/missionmaterials/library-missionmaterials.php");',1000);
                            }
                            else
                            {
				...
				$.ajax({
					...
					success:function(data) {
						...
						if(data=="success")
						{
							...
							setTimeout('closeloadingalert()',1000);
							setTimeout('removesections("#library-missions");',500);
							setTimeout('showpages("library-missionmaterials","library/missionmaterials/library-missionmaterials.php");',500);
						}
						else
						{
