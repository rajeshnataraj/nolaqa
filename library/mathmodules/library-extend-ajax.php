...
if($oper=='extendtxtform' and  $oper!='')
{
	...
	$qryforgetmoduledetails= $ObjDB->QueryObject("SELECT fld_mathmodule_name as modulename,fld_id as decryptmoduleid 
	...
	if($type=='rename' or $type=='copy')
	{
		$extendtxt= $ObjDB->SelectSingleValue("SELECT fld_extend_text 
		                                       FROM itc_extendtextmath_master WHERE fld_id='".$extendid."'");
	}
	switch($type)
	{
	 case 'new':
...
if($oper=='saveextendtxt' and  $oper!='')
{
	...
	{	
		...
		$extendid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_extendtextmath_master
		...
		$qryforgetmoduledetails= $ObjDB->QueryObject("SELECT a.fld_mathmodule_name AS modulename ,b.fld_file_name AS modulefilename 
		...
		{
			$ObjDB->NonQuery("UPDATE itc_extendtextmath_master 
...
if($oper=='deleteextend' and  $oper!='')
{
	...
	try {
		...
		$ObjDB->NonQuery("UPDATE itc_extendtextmath_master_mapping_text 
		                 SET fld_delstatus='1',fld_deleted_by='".$uid."',fld_deleted_date='".$date."' 
...
if($oper=='saveextendguidetips' and  $oper!='')
{
	...
	if($validate_moduleid and  $validate_extendid){
		...
		$id=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_extendtextmath_master_mapping_text(fld_module_id,fld_extend_content_text,
...
if($oper=='getextendguidetips' and  $oper!='')
{
	...
	$qryforextenttxt=$ObjDB->QueryObject("SELECT fld_extend_content_text,fld_id,fld_created_by 
	                                     FROM itc_extendtextmath_master_mapping_text 
...
if($oper=='checkextendcontent' and  $oper!='')
{
 ...
 WHERE fld_ext_id='".$id."' AND fld_active='1')+ (SELECT COUNT(fld_id) FROM `itc_class_indassesment_extcontent_mapping`  WHERE fld_ext_id='".$id."' AND fld_active='1')");
