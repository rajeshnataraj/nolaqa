...
/*
	...
	Page - library-mathmodules-Grade
	Description:
		Show the grades based on mathmodules
...
$qrymath = $ObjDB->QueryObject("SELECT CONCAT(a.fld_mathmodule_name,' ',b.fld_version) AS msg, a.fld_module_id AS mathmodid
				                            FROM itc_mathmodule_master AS a 
											LEFT JOIN itc_module_version_track AS b ON b.fld_mod_id=a.fld_module_id
											WHERE a.fld_id='".$id."' AND b.fld_delstatus='0' AND a.fld_delstatus='0'");
...
<section data-type='#library-mathmodules' id='library-mathmodules-grade'>
    <div class='container'>
        <div class='row'>
                ...
                <form name="moduleforms" id="moduleforms">
					...
					if($sessmasterprfid!=2 and $sessmasterprfid!=3)
					{
						$qry = $ObjDB->QueryObject("SELECT fld_id, fld_page_title AS title, fld_preassment_id AS pageid, fld_grade AS grade, fld_points AS points, 
													...
													WHERE fld_module_id='".$id."' AND fld_flag='1' AND fld_school_id='".$schoolid."' AND fld_user_id='".$indid."' AND fld_schedule_type='2' 
					...
					if($qry->num_rows<=0 or $sessmasterprfid==2 or $sessmasterprfid==3)
                    {
						...
						$qry = $ObjDB->QueryObject("SELECT fld_id, fld_page_title AS title, fld_preassment_id AS pageid, fld_grade AS grade, fld_points AS points, 
														fld_session_id AS sessionid 
													FROM itc_module_wca_grade 
													WHERE fld_module_id='".$id."' AND fld_flag='1' AND fld_school_id='0' AND fld_user_id='0' AND fld_schedule_type='2' 
					...
					if($qry->num_rows<=0)
                    {
						$qry = $ObjDB->QueryObject("SELECT fld_id, fld_page_title AS title, '0' AS pageid, fld_grade AS grade, fld_points AS points, fld_session_id AS sessionid 
												  ...
												  WHERE fld_module_id='".$mathmodid."' AND fld_flag='1'		
													UNION ALL 		
												  ...
												  WHERE fld_module_id='".$mathmodid."' AND fld_performance_name<>'Total Pages' AND fld_delstatus='0' 
