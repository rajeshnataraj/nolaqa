...
if($oper=='extendtxtform' and  $oper!='')
{
	...
	$qryforgetexpdetails= $ObjDB->QueryObject("SELECT fld_exp_name as expname,fld_id as decryptexpid 
	                                             FROM itc_exp_master 
	...
	if($type=='rename' or $type=='copy')
	{
		$extendtxt= $ObjDB->SelectSingleValue("SELECT fld_extend_text 
		                                       FROM itc_exp_extendmaterials_master WHERE fld_id='".$extendid."'");
	}
	switch($type)
	{
	 case 'new':
...
if($oper=='saveextendtxt' and  $oper!='')
{
	...
	if($validate_materialid AND  $validate_extendid)
	{
		...
		if($type=='new' or $type=='copy')
		{
		 ...
		 $created_name=$ObjDB->SelectSingleValue("SELECT CONCAT(fld_fname,' ',fld_lname) AS createdname 
                 ...
                 if($type=='copy')  {
                     ...
                     $qryselectcopy_map =$ObjDB->QueryObject("SELECT M.fld_extend_id as mextendid,M.fld_expedition as mexpid,M.fld_destination as mdestn,M.fld_task as mtask,M.fld_material as mmaterial FROM itc_exp_extendmaterials_mapping AS M
                                                                WHERE M.fld_extend_id='".$extendid."' AND M.fld_expedition='".$materialid."' AND M.fld_created_by='".$uid."' AND M.fld_delstatus='0'");
                     
		$extendid=$ObjDB->NonQueryWithMaxValue("INSERT INTO itc_exp_extendmaterials_master
                       ...
                       if($qryselectcopy_map->num_rows > 0)   {
                            ...
                            {
                                    extract($row);
                                    $expmatlistid = $ObjDB->NonQueryWithMaxValue("INSERT INTO itc_exp_extendmaterials_mapping(fld_extend_id, fld_expedition, fld_destination, fld_task, fld_material, fld_created_date, fld_created_by)
		...
		{
			$ObjDB->NonQuery("UPDATE itc_exp_extendmaterials_master 
...
if($oper=='checkextendcontent' and  $oper!='')
{
	...
	try{
	 ...
	 $chk = $ObjDB->SelectSingleValueInt("SELECT COUNT(fld_id) FROM `itc_class_indasexpedition_extcontent_mapping` 
...
if($oper=='deleteextend' and  $oper!='')
{
	...
	try {
		...
		$ObjDB->NonQuery("UPDATE itc_exp_extendmaterials_master SET fld_delstatus='1',fld_deleted_by='".$uid."',fld_deleted_date='".date("Y-m-d H:i:s")."'  
                                    WHERE fld_id='".$extendid."'");
								   
		$ObjDB->NonQuery("UPDATE itc_exp_extendmaterials_mapping SET fld_delstatus='1',fld_deleted_by='".$uid."',fld_deleted_date='".date("Y-m-d H:i:s")."' 
						 WHERE fld_extend_id='".$extendid."'  ");
						 
                if($schflag==1)
                {
                       $ObjDB->NonQuery("UPDATE itc_class_indasexpedition_extcontent_mapping SET fld_active='0', fld_updatedby='".$uid."', fld_updateddate='".date("Y-m-d H:i:s")."' where fld_ext_id='".$extendid."'"); 
                }
	  echo "sucess";
		}
		else
        ...
        <dt class='dropdown'>   
            ...
            <div class="selectbox-options" style="width:210px;">
             <input type="text" class="selectbox-filter" placeholder="Search Task" style="width:180px;">
                 <ul role="options">
                     <?php
                         $qrymaterial =  $ObjDB->QueryObject("SELECT fld_id as taskid, fld_task_name as taskname, fn_shortname (CONCAT(fld_task_name), 1) AS shortname, fld_task_desc, fld_task_id 
                                                    FROM itc_exp_task_master WHERE fld_dest_id='".$destnid."' AND fld_flag='1' AND fld_delstatus='0'"); 
                  ...
                  <div class="selectbox-options" style="width:210px;">
                        ...
                        <ul role="options">
                           <?php
                               ...
                               $qrymaterial =  $ObjDB->QueryObject("SELECT fld_id as materialid, fld_materials as material, fn_shortname (CONCAT(fld_materials), 1) AS shortname FROM itc_materials_master
                                                                                                        WHERE fld_delstatus='0' AND fld_created_by='".$uid."' AND fld_id NOT IN(SELECT fld_material FROM itc_exp_extendmaterials_mapping where fld_destination='".$destnid."' AND fld_task='".$taskid."' AND fld_extend_id='".$extndid."')");
                                    if($qrymaterial->num_rows > 0){
                                            while($rowsqry = $qrymaterial->fetch_assoc()){
                                                    extract($rowsqry);     ?>
    ...
    try  
    {
        if($expnmaterialid == 0)
        {
          ...
          echo "sucess";
            ...
            $ObjDB->NonQuery("UPDATE itc_exp_extendmaterials_mapping SET fld_extend_id='".$extendid."', fld_expedition='".$expnid."', fld_destination='".$destnid."', fld_task='".$taskid."', fld_material='".$materialid."', 
...
if($oper=='showdefineexpmateril' and  $oper!='')
{
  ...
  $qry_expdetailslist= $ObjDB->QueryObject("SELECT S2.fld_dest_name as destname,S3.fld_task_name as taskname,S4.fld_materials as materialname,S1.fld_id as expextmatid,S1.fld_destination as destnid,S1.fld_task as taskid, S1.fld_material as materialid
                                                FROM itc_exp_extendmaterials_mapping AS S1 
                                              INNER JOIN itc_exp_destination_master AS S2 ON S2.fld_id = S1.fld_destination
                                              INNER JOIN itc_exp_task_master AS S3 ON S3.fld_id = S1.fld_task
                                              INNER JOIN itc_materials_master AS S4 ON S4.fld_id = S1.fld_material where S1.fld_id='".$expmatid."'");
...
{		
        $expmatid = isset($method['expmatid']) ? $method['expmatid'] : '0';       
        $ObjDB->NonQuery("UPDATE itc_exp_extendmaterials_mapping SET fld_delstatus='1',fld_deleted_date='".date("Y-m-d H:i:s")."',fld_deleted_by='".$uid."' WHERE fld_id='".$expmatid."'");
