...
$qrytag = $ObjDB->QueryObject("SELECT a.fld_id AS tagid, a.fld_tag_name AS tagname 
							  ...
							  AND a.fld_delstatus='0' AND b.fld_item_id='".$expeditionid."'   
							  GROUP BY a.fld_id ");

if($expeditionid==0){
	$createbtn = "Create Expedition";
	$cancelbtn = "Cancel";
	$expeditionname = "";
	$expeditionversion = "";
	$filename = "";
	$type = "";
	$msg = "New Expedition";
	$cancelclick="fn_cancel('library-expedition')";
        $expdescr ='';
         $expuiid=1;
         $expuiname="Standard UI";
}
else{
	$createbtn = "Update Expedition";
	$cancelbtn = "Cancel";

	$expeditionqry = $ObjDB->QueryObject("SELECT a.fld_exp_name, a.fld_asset_id, a.fld_ui_id, b.fld_version, b.fld_file_name, b.fld_zip_type,a.fld_exp_desc
											FROM itc_exp_master AS a 
											LEFT JOIN itc_exp_version_track AS b ON a.fld_id = b.fld_exp_id 
											WHERE a.fld_id='".$expeditionid."' AND a.fld_delstatus='0' 
												AND b.fld_delstatus='0' GROUP BY a.fld_exp_name");

	while($rowexpedition=$expeditionqry->fetch_assoc())
	{
		extract($rowexpedition);
		$expeditionname = $fld_exp_name;
		$expeditionversion = $fld_version;
		$filename = $fld_file_name;
		$type = $fld_zip_type;
		$assetid = $fld_asset_id;
		$expdescr= $fld_exp_desc;
                $expuiid = $fld_ui_id;
...
<script language="javascript" type="text/javascript" charset="utf-8">
	...
	$(function(){				
		...
		<?php 
                    if($expeditionid != '' and $expeditionid!='0'){
				$qrytag = $ObjDB->QueryObject("SELECT a.fld_id AS tagid,a.fld_tag_name AS tagname 
				                              FROM itc_main_tag_master AS a, itc_main_tag_mapping AS b 
											  WHERE a.fld_id=b.fld_tag_id AND b.fld_tag_type='28' 
											        AND b.fld_access='1' AND a.fld_created_by='".$uid."' 
													AND a.fld_delstatus='0' AND b.fld_item_id='".$expeditionid."'");
...
<?php }?>
<section data-type='2home' id='library-expedition-newexpedition'>
    <div class='container'>
                ...
                <form name="expeditionforms" id="expeditionforms">
                    ...
                    <div class='row'>
                        <div class='six columns'>
                         	Select Expedition Content<span class="fldreq">*</span>
                            <div class='row'>
                            	<input type="file" name="file_upload_expedition" id="file_upload_expedition" />
                                <div id="queue" ></div>
                        ...
                        <div class='three columns'>
                            ...
                            <div class='row'>
                                <div class="selectbox" id="expeditionselectbox">
                                    <input type="hidden" name="selectversion" class="required" id="selectversion" value="<?php echo $expeditionversion;?>" >
                                    <a class="selectbox-toggle" role="button" data-toggle="selectbox" href="#">
                                        <span class="selectbox-option input-medium" data-option="<?php echo $expeditionversion;?>" id="versions"  style="width:95%;">Version <?php echo $expeditionversion;?></span>
                                        <b class="caret1"></b>
                                    </a>
                                    <?php if($expeditionid!=0){?>
                                    <div class="selectbox-options">			    
                                        <ul role="options">
                                           <?php $qry = $ObjDB->QueryObject("SELECT fld_version FROM itc_exp_version_track WHERE fld_exp_id='".$expeditionid."'");
                                            while($res = $qry->fetch_object()){?>
                                               <li><a  href="#" data-option="<?php echo $res->fld_version; ?>" onclick="fn_changeexpeditionname(<?php echo $res->fld_version;?>)">Version <?php echo $res->fld_version; ?></a></li>
                                            <?php }?>                                                   
                                        </ul>
                                    </div>
                                    <?php }?> 
                    ...
                    <div class='row  rowspacer'>
                        <div class='six columns'>
                        	New Expedition Name<span class="fldreq">*</span>
                            <dl class='field row'>
                                <dt class='text'>
                                    <input placeholder='New Expedition Name' type='text' id="txtexpname" name="txtexpname" value="<?php echo $expeditionname;?>" onBlur="$(this).valid();" />
                                </dt>
                            </dl>
                      ...
                      Description
                             <dl class='field row'>
                                <dt class='textarea'>
                                    <textarea placeholder='Tell us about your new expedition' id="expdescription" name="expdescription"  style="height:315px; width:100%; border-color:#FFF; resize:none;"
><?php echo htmlentities($expdescr); ?></textarea>
                    ...
                    <script language="javascript" type="text/javascript">
                        ...
                        $(document).ready(function() { 
						  ...
						  <?php $timestamp = time();?>
						  $('#file_upload_expedition').uploadify({
								...
								'fileTypeDesc'  : 'Expedition Conent',
								'buttonText' : 'Select Expedition Content',
								...
								'onFallback' : function() {
								 ...
								 'onUploadProgress' : function(file, bytesUploaded, bytesTotal, totalBytesUploaded, totalBytesTotal) {
                                       $('#createexpeditionbtn').addClass('dim');   
                                    },
								'onUploadSuccess'  : function(file, data, response) {
									 console.log(data);
									 
									 $('#createexpeditionbtn').removeClass('dim');
									 ...
									 if(upres[0] == 'success')
									 {
										...
										$('#versions').html("Version "+upres[2]);
                                                                                ...
                                                                                tinyMCE.get('expdescription').setContent(upres[4]);
									...
									if(upres[0] != 'success') {
										 ...
										 if(upres[0] == "invalid") {
										 	msg="Please upload a valid Expedition Content File";		 
										 }
										 else {
                 ...
                 <script type="text/javascript" language="javascript">
                        ...
                        <div class='six columns'>
                                    ...
                                    <?php
                                        $qrystudent= $ObjDB->QueryObject("SELECT a.fld_id,a.fld_license_name AS licensename, 
                                                                                        ...
                                                                                        WHERE a.fld_id NOT IN(SELECT fld_license_id FROM itc_license_exp_mapping WHERE fld_exp_id='".$expeditionid."'
                                                                                         AND fld_delstatus='0')
                                            ...
                                            <div id="list10" class="dragleftinner droptrue1">
                                             <?php                         
                                              ...
                                              $qrystudent= $ObjDB->QueryObject("SELECT a.fld_id,a.fld_license_name AS licensename, 
                                                                                            a.fld_license_name AS shortname 
                                                                                            FROM itc_license_master As a
                                                                                            WHERE a.fld_id IN(SELECT fld_license_id FROM itc_license_exp_mapping WHERE fld_exp_id='".$expeditionid."'
                                                                                            AND fld_delstatus='0')
                                                                                            AND a.fld_delstatus='0' AND a.fld_license_type='1' 
                                                                                            ORDER BY licensename ASC");
                    ...
                    <div class='row'>
                        ...
                        <div id="createmodulebtn" class='four columns btn secondary yesNo'>
                            <a onclick="fn_createexpedition(<?php echo $expeditionid;?>)"><?php echo $createbtn;?></a>
                        </div>
                    </div>
                    <?php }?>
                </form>
