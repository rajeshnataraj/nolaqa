...
$missionid = isset($method['id']) ? $method['id'] : '0';

$missionname = $ObjDB->SelectSingleValue("SELECT CONCAT(a.fld_mis_name,' ',b.fld_version)
												FROM itc_mission_master AS a 
												LEFT JOIN itc_mission_version_track AS b ON a.fld_id = b.fld_mis_id 
												WHERE a.fld_id='".$missionid."' AND a.fld_delstatus='0' AND b.fld_delstatus='0'");
?>

<section data-type='2home' id='library-mission-toggle'>
    <div class='container'>
    	<!--Load the Expedition Name-->
        <div class='row'>
            <div class='twelve columns'>
            	<p class="dialogTitle"><?php echo $missionname; ?></p>
            ...
            <div class='twelve columns centered insideForm'>
                <form name="exporderforms" id="exporderforms">
                        ...
                        $qrydestinations = $ObjDB->QueryObject("SELECT fld_id AS destid, fld_dest_name AS destname, fld_order AS destorder, fld_next_order AS nextdestorder, fld_toggle_status AS togstatus, fld_dest_status AS deststatus
                                                                ...
                                                                WHERE fld_mis_id='".$missionid."' AND fld_flag='1' AND fld_delstatus='0' ORDER BY fld_order"); 
                        $destcnt = 0;
                        ...
                        <table class='table table-striped table-bordered setbordertopradius' id="expordertable" width="100%">
                            <thead class='tableHeadText'>
                                <tr>
                                    <th class='centerText' style="width: 30%">Title</th>
                                    <th class='centerText' style="width: 25%">Status</th>
                                </tr>
                            </thead>
                        </table>
                        
                        <div style="max-height:500px;width:auto;" id="tablecontents15"  >
                        <table style="margin-bottom:0px;" class='table table-striped table-bordered bordertopradiusremove'>
                        <tbody>
                        <?php
                        for($i=0;$i<$destcnt;$i++)
                        {
                            $dstatus = $ObjDB->SelectSingleValueInt("SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_dest_id='".$destid[$i]."' AND fld_school_id='".$schoolid."' AND fld_created_by='".$uid."' AND fld_user_id='".$indid."'");
                            
                            if($dstatus=='' or $dstatus=='0' or $dstatus==NULL)
                            {
                                $dstatus = $ObjDB->SelectSingleValue("SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_dest_id='".$destid[$i]."' AND fld_school_id='0' AND fld_user_id='0' AND fld_flag='1'");
                            ...
                            <tr>
                                <td style="width: 30%">
									...
									<div><?php echo $i+1;?>. <?php echo $destname[$i]; ?></div>
                                </td>
                            ...
                            for($j=0;$j<$taskcnt;$j++)
                            {
                                $tstatus = $ObjDB->SelectSingleValueInt("SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_task_id='".$taskids[$j]."' AND fld_school_id='".$schoolid."' AND fld_created_by='".$uid."' AND fld_user_id='".$indid."'");
                            
                                if($tstatus=='' or $tstatus=='0' or $tstatus==NULL)
                                {
                                    $tstatus = $ObjDB->SelectSingleValue("SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_task_id='".$taskids[$j]."' AND fld_school_id='0' AND fld_user_id='0' AND fld_flag='1'");
                                ...
                                <tr style="display:none;">
                                    <td style="width: 30%">
										<div style="margin-left:50px;"><?php echo $i+1;?>.<?php echo $j+1;?>.<?php echo $taskname[$j]; ?></div>
                                    </td>
                                    <td style="width: 25%" <?php if($tasktoggle[$i]==1 and $sessprofileid != 2 and $tstatus==3) {?>class='dim'<?php } ?>>
							...
							<?php
                                ...
                                for($k=0;$k<$rescnt;$k++)
                                {
									...
									//echo "SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_res_id='".$resids[$k]."' AND fld_school_id='".$schoolid."' AND fld_user_id='".$indid."'";
                                    $status = $ObjDB->SelectSingleValueInt("SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_res_id='".$resids[$k]."' AND fld_school_id='".$schoolid."' AND fld_created_by='".$uid."' AND fld_user_id='".$indid."'");
                            
                                    if($status=='' or $status=='0' or $status==NULL)
                                    {                                        
										$status = $ObjDB->SelectSingleValueInt("SELECT fld_status FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_res_id='".$resids[$k]."' AND fld_school_id='0' AND fld_user_id='0' AND fld_flag='1'");
                                    ...
                                    <tr>
                                    	<td style="width: 30%; padding-left:50px;">
                                            <div><?php echo $i+1;?>.<?php echo $j+1;?> <?php echo $resname[$k]; ?></div>
                                        </td>
        ...
        <div class="row rowspacer" style="margin-top:20px;">
                ...
                <div class="tRight">
                    <input type="button" class="darkButton" style="width:200px; height:42px;float:right;margin-bottom:20px;margin-right:20px;" value="Save Status" onClick="fn_saveresstatus(<?php echo $missionid; ?>);" />
                </div>
                <?php 
                $chkdata = $ObjDB->SelectSingleValueInt("SELECT count(fld_id) FROM itc_mis_res_status WHERE fld_mis_id='".$missionid."' AND fld_school_id='".$schoolid."' AND fld_created_by='".$uid."' AND fld_user_id='".$indid."'");
                if($sessprofileid != 2 and $chkdata !=0){ ?>
                <div class="tRight">
                    <input type="button" class="darkButton" style="width:200px; height:42px;float:right;margin-bottom:20px;margin-right:20px;" value="Reset to Default" onClick="fn_resetdft(<?php echo $missionid;?>,<?php echo $uid ?>,<?php echo $schoolid ?>,<?php echo $indid ?>);" />
                </div>
                <?php } ?>
            </div>
        </div>
