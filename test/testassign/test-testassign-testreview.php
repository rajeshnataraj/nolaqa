<?php 
	...
	$qrytestdetails = $ObjDB->QueryObject("SELECT `fld_test_name` AS testname, `fld_test_des` AS testdes, `fld_time_limit` AS testtime, fld_score AS score, 
	                                     `fld_max_attempts` AS testattempts FROM `itc_test_master` WHERE 
										 fld_id='".$id[0]."' AND `fld_delstatus`='0'");
		$rowtestdetails = $qrytestdetails->fetch_assoc();
		extract($rowtestdetails);
		
	
?>
<section data-type='#test-testassign' id='test-testassign-testreview'>
	...
	<div class='container'>
        <div class='row'>
            <div class='twelve columns'>
            	...
            	<p class="dialogSubTitleLight">Review your assessment details below, then click Save Assessment to complete this wizard process.</p>
            </div>
        </div>
                ...
                <div class="row">
                    ...
                    <div class="eight columns">
                        ...
                        <div class="row rowspacer">
                            ...
                            <?php $qryqusdetails = $ObjDB->QueryObject("SELECT a.fld_question_id AS qid, b.fld_question AS qusname 
							                                           FROM `itc_test_questionassign` AS a, `itc_question_details` AS b 
																	   WHERE a.fld_question_id=b.fld_id AND a.fld_test_id='".$id[0]."' AND a.fld_delstatus='0' 
                            ...
                            if($qryqusdetails->num_rows>0){
                                ...
                                while($rowqusdetails = $qryqusdetails->fetch_assoc())
                                {
                                    ...
                                    ?>
                                    <div class="wizardReportData" onclick="fn_showquestion(<?php echo $id[0];?>,<?php echo $qid;?>);" style="width:550px;cursor:pointer;"><?php echo  $j.".".trim(str_replace("&nbsp;", "",$tempqusname));?></div>
                                <?php
                                $j++;
                                }
                            }
                ...
                <div class='row rowspacer'>
                    <div class='row'>
                        <p class='btn secondary four columns' style="margin-left:31%">
                           <a onclick="fn_savetest(<?php echo $id[0];?>,<?php echo $id[1];?>)">Save Assessment</a>
                        </p>
                    </div>
                </div>
            </div>
