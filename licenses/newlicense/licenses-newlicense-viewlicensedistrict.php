<?php 
/*
	Page - licenses-newlicense-viewlicensedistrict
	Description:
	This is used for show the license detatils for particular district and add another licnese
...
$id = explode(',',$id); //id[0]->districtid, id[1]->licenseid

$disrtict_id = $id[0];
$templicenseid = $id[1]; // comes from licenses-newlicense-actions.php

//get districtname, licensecount
$getbasicdetails = $ObjDB->QueryObject("SELECT (SELECT fld_district_name FROM itc_district_master WHERE fld_id='".$disrtict_id."') AS distname, 
											  (SELECT COUNT(fld_id) FROM itc_license_master  WHERE fld_delstatus='0' AND fld_license_type='1') AS totalhidlicense,
											  (SELECT COUNT(fld_id) FROM itc_license_track WHERE fld_district_id='".$disrtict_id."' AND fld_school_id=0 AND fld_delstatus='0') 
											    AS licensecount");
...
<section data-type='license' id='licenses-newlicense-viewlicensedistrict'>
	<div class='container'>
    	<div class='row'>
          <div class='twelve columns'>
            <p class="dialogTitle"><?php echo $distname; ?></p>
            ...
            <div class='eleven columns centered insideForm'>
                  <form method='post' name="distval" id="distval">                                       
                    <div class="row" id="addlicensedist"> 
                        <?php 
                            ...
                            $distqry = $ObjDB->QueryObject("SELECT a.fld_id AS trackid, a.fld_license_id AS licenseid,a.fld_renewal_count, b.fld_license_name AS licensename, 
																fn_shortname(b.fld_license_name,2) AS licenseshortname, 
														   ...
														   WHERE a.fld_district_id='".$disrtict_id."' AND a.fld_school_id='0' AND a.fld_user_id='0' AND a.fld_delstatus='0'");						
                    ...
                    <div class="row rowspacer">
                        <div class="four columns">
                            <p class='btn medium <?php if($totalhidlicense==$licensecount or $disrtict_id==0) echo "dim";?>' id="add">
                                <a onclick="addlicdist($('#hidaddlicense').val());">Add another License</a>
                            </p> 
                        </div>
                    </div>
                    <input type="hidden" id="hidaddlicense" value="<?php if($disrtict_id==0)echo '1'; else echo $count;?>" />
                    <input type="hidden" id="hiddistid" value="<?php echo $disrtict_id; ?>" />
                    ...
                    <div class='row'>
                        ...
                        <div class='six columns'>                            
                            <p class='btn secondary twelve columns'>
                                <a onclick="fn_updatelicense(<?php echo $disrtict_id;?>,'district',<?php echo $templicenseid;?>)">Update</a>
                            </p>                           
                        </div>
                    </div>
                </form>
