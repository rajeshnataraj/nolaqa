...
function fn_showclass(schoolid)
{
	...
	$.ajax({
		type: 'post',
	     url: 'reports/digitalrubricmission/reports-digitalrubricmission-ajax.php',
		data: dataparam,
		beforeSend: function(){
...
function fn_showexp(schid)
{
 ...
 var dataparam = "oper=showexpedition&schid="+schid+"&clsid="+clsid;	
	$.ajax({
		...
		data: dataparam,
		beforeSend: function(){
			$('#expdiv').html('<img src="img/loader.gif" width="200"  border="0" />'); 	
		},
		success:function(data) {
			$('#showexp').show();	
...
function fn_showrubric(expid,schid)
{
  	$('#viewreportdiv').hide();
	$('#studentdiv').hide();
    $('#viewreportdiv').hide();
...
function fn_digitalrubric()
{
    ...
    setTimeout('removesections("#reports-digitalrubricmission");',500);
    oper="digitalrubricmissionreport";
