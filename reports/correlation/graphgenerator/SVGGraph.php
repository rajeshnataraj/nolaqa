...
class SVGGraph {
  ...
  public function Render($class, $header = TRUE, $content_type = TRUE,
    $defer_js = FALSE,$type,$id)
  {
    $this->g = $this->Setup($class);
    return $this->g->Render($header, $content_type, $defer_js,$type,$id);
  }
...
abstract class Graph {
  ...
  public function Render($header = TRUE, $content_type = TRUE, 
    $defer_javascript = FALSE,$type,$id)
  {
    ...
    try {
      ...
      if($content_type)
        header($mime_header);
      // echo $content;
	 file_put_contents('reportimages/someFile'.$type.'_'.$id.'.svg', $content);
    } catch(Exception $e) {
      if($content_type)
        header($mime_header);
     $this->ErrorText($e);
