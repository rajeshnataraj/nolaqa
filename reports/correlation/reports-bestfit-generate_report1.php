...
// To get the guid of grades for the selected standards
		for($g=0;$g<sizeof($standardids);$g++)
		{
			...
			//$grades = $xpath->query("//itm[@type='grade']");
			//$guids = $xpath->query("//itm[@type='grade']//itm[@type='standard']/*");
...
//                       echo sizeof($allstandardsguid[$standardids[$g]])."listofsize";
...
// To calculate the standard/benchmark addressed summary 
		...
		//$mainstdarray = array();
                ...
                //echo sizeof($productid)."percentage";
		$tempbstdarray = array();
		for($k=0;$k<sizeof($standardids);$k++)
		{
			...
			for($p=0;$p<sizeof($productid);$p++)
			{
				...
				foreach($stdproduct as $key => $val) {
					...
					$result = array_intersect($stdproduct[$key],$psalign);// common values of both array
					...
					if(sizeof($result) > 0) {
						$totalalign++;
						$resultarray[]=array_values($result);//set key start with 0,1,2..
                                                //print_r($resultarray);
				...
				unset($psalign);
                         ...
                         if($type==1){
                                              //print_r($tempstdarray);
                                              echo "outer";
                                          }
                       // echo "completed";
                        $finalresultarray[]=$resultarray;  
//                       print_r($resultarray);
//                        echo "ssn".sizeof($resultarray);
			...
			{
                           // echo " temparray ".(sizeof(array_unique($tempstdarray)))."mainids: ".(sizeof(array_unique($mainstdarray)));
//                             if($type==1){
//                        print_r($tempstdarray);
//                        echo " temparray ";print_r($mainstdarray); echo "mainids: ";
//                         }
...
/*For grade*/	
	   ...
	   for($x=0;$x<sizeof($standardids);$x++)
	{
		...
		/*$grades = $xpath->query("//itm[@type='grade']//itm[@type='standard']/meta");
		$guids = $xpath->query("//itm[@type='grade']//itm[@type='standard']");
		$gradenameqry = $xpath->evaluate('//itm[@type="grade"]');*/
 ...
 /** Percentage calculation for multiple standards with products **/
   ...
   // percentage for product titles
                    ...
                    for($o=0;$o<$lenprd;$o++){
                       ...
                       // print_r($percenatageforstdinv);
					$perceforsel=array();
                                        
                        for($y=0;$y<sizeof($percenatageforstdinv);$y++)
                        {
                           //echo $percenatageforstdinv[$y]."/".$standardindividualcount[$y];
...
/** standardindividualcount calculation for max percentage standard **/	
	   ...
	   for($totalcnt=0;$totalcnt<$stdlen;$totalcnt++)
	{
        ...
        //print_r($standardindividualcount);

/** for best fit highest recommendation calculation **/


if($checbox==0){ // not required products 
