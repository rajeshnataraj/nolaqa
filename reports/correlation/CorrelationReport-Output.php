...
/**for purpse of calcuating standards address summary **/
	function getpercentage($standard,$proid,$statename,$repid,$type)
	{
		...
		for($g=0;$g<sizeof($standardids);$g++)
		{
			...
			$string = file_get_contents("standards/".$standardids[$g].".xml");				
			$doc = new DOMDocument();
			$doc->loadXML($string);
			...
			//$element->query("ancestor::itm/@guid");
			//echo $guids->length;
		...
		for($h=0;$h<sizeof($productid);$h++)
		{
			...
			$string = file_get_contents("products/".$productid[$h].".xml");	
	...
	$colorlight=array("#fdddce","#dddaee","#d1d8ce","#dcd6eb","#d1d7e5","#ccddee");
        ...
        $qry=$ObjDB->QueryObject("SELECT a.fld_doc_name AS standardname, a.fld_doc_id AS standardid, a.fld_createddate, a.fld_std_body, b.fld_name AS statename,
                                    c.fld_report_style AS style,c.fld_prepared_on as date
                                    FROM itc_correlation_rpt_doc_mapping AS a
                                    LEFT JOIN itc_standards_bodies AS b on a.fld_std_body=b.fld_id
                                    LEFT JOIN itc_correlation_report_data AS c on a.fld_cor_id=c.fld_id
                                    WHERE c.fld_id='".$id."' AND a.fld_flag='1'");
       $stdnamefordoc=array();
       $standardnam='';
        $m=0;
	if($qry->num_rows > 0){
		...
		extract($rowqry);
                ...
                $stdnamefordoc[]=$standardname."~".$standardid;
                $comma = ",";
				$m++;
				if($m==$qry->num_rows)
				{
	...
	$qryforproducts=$ObjDB->QueryObject("SELECT b.fld_prd_name AS prodctname, b.fld_prd_id AS prductid, b.fld_prd_asset_id AS assetid
	...
	for($x=0;$x<sizeof($standardids);$x++)
	{
		...
		$string = file_get_contents("standards/".$standardids[$x].".xml");	
		$doc = new DOMDocument();
		$doc->loadXML($string);
		$xpath = new DOMXpath($doc);
		$coursecnt=$xpath->evaluate('count(//itm[@type="course"])');
...
<p align="right" style="color:<?php echo $colors[$colorstyle];?>;"><h1><?php echo $schoolname; ?></h1></p>
	...
	?>
		...
		<table width="100%" style="background-color:<?php echo $colors[$colorstyle];?>;color:white;" cellspacing="0" cellpadding="8">
			<tr>
            	<td width="70%"><b>STANDARD SETS</b></td>
                <td width="30%"><b>TITLE SET</b></td>
          	</tr>
		</table>
		<?php
		for($app=0;$app<sizeof($stdnamefordoc);$app++){
	$stddet=explode('~',$stdnamefordoc[$app]);
		$sub=explode('|',$stddet[0]);
		...
		<table>
            ...
            <tr>
				<td width="70%">
					...
					$qryforgrad=$ObjDB->QueryObject("SELECT a.fld_sub_guid AS standardid,b.fld_grade as grad
													FROM itc_correlation_doc_subject as a 
													LEFT JOIN itc_correlation_rpt_std_grades AS b on a.fld_id=b.fld_sub_id
													WHERE b.fld_rpt_data_id='".$id."' AND b.fld_delstatus='0'");
					$z=0;
                	 while($rowqryforgra = $qryforgrad->fetch_assoc())
                         {
			        	extract($rowqryforgra);
					
					if($stddet[1]==$standardid)
					{
					
		                       ?>
		                        <font style="color:#606060;" ><?php if($z>0){echo ",".$grad;}else{ echo $grad; }  ?></font>
                                    <?php
		                   $z++;      
...
?>
		<p align="left" style="line-height:1;font-size:35px;color:#606060;"><b>Please Note</b></p>
			...
			<table width="100%" border="0.5" cellpadding="8">
				<thead>
					<tr style="background-color:<?php echo $colors[$colorstyle] ?>" > 
						...
						<td  width="75%" style="color:#FFF;font-weight:bold;"><?php echo key($finalstandardlist[$z]); ?><br/><?php echo $statename."|".$stdname; ?></td> 
					</tr>
				</thead> 
				<?php 
...
$desccheck =$ObjDB->SelectSingleValueInt("SELECT fld_sec_prod_description
									  	  FROM itc_correlation_report_data where fld_id='".$id."'");
if($desccheck==1){
...
} //for($q=0;$q<sizeof($productname);$q++) closed
