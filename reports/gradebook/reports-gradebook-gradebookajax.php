    ...
    <table class="fancyTable" id="myTable05" cellpadding="0" cellspacing="0" style="padding-bottom:20px;">
        <thead>
            <tr>
                ...
                ?>
                <th align="center" class="stuhead"><span style="font-size:14px;">Student Name</span></th>
        ...
        $qryrub = $ObjDB->QueryObject("SELECT a.fld_schedule_id AS scheduleid, a.fld_rubric_id AS rubricids, fn_shortname(CONCAT(a.fld_rubric_name,' / Rubric'),1) AS nam, 
                                                CONCAT(a.fld_rubric_name) AS rubnam FROM itc_class_expmis_rubricmaster AS a 
                                                        ...
                                                        LEFT JOIN itc_class_indasexpedition_master AS d ON a.fld_schedule_id=d.fld_id 
                                                                WHERE a.fld_class_id='".$classid."' AND a.fld_schedule_id='".$scheduleid."' AND a.fld_delstatus='0'   
...
//Assessment ends  
    ...
    <script language="javascript" type="text/javascript">
            ...
            <?php if($qryhead->num_rows<=0){?>
                    ...
                    $(".studentth").css('height','38px');
                    $(".studentth").css('max-width','209px');
            <?php }
            else
            {
            ?>
             $(".stuhead").css({"width":"209px"});
             <?php
            }
            ?>
    </script>
...
//math connection
	...
	else if($type==23) // Mission Schedule
	{
            ...
            if($mistypeid[$i]!='') {
                $cnt = $ObjDB->SelectSingleValueInt("SELECT fld_id FROM itc_mis_points_master 
                                                                            WHERE fld_schedule_type='" . $type . "' AND fld_schedule_id='" . $scheduleid . "' 
                                                                                    AND fld_mis_id='" . $unitmodid . "' AND fld_student_id='" . $studentid . "' 
                                                                                    AND fld_mistype='" . $mistype[$i] . "' AND fld_res_id='" . $mistypeid[$i] . "' AND fld_delstatus='0'");
                //when mispoint is blank or '', put a null in the database
                $mispointsqlfragment = "'" . intval($mispoint[$i]) . "'";
                if ($mispoint[$i] == "''"){
                    $mispointsqlfragment = " NULL ";
                }

                if ($cnt != '') {
                    $ObjDB->NonQuery("UPDATE itc_mis_points_master SET fld_teacher_points_earned=$mispointsqlfragment, fld_points_possible='" . $mispossible[$i] . "', fld_lock='1', fld_updated_by='" . $uid . "', fld_updated_date='" . date("Y-m-d H:i:s") . "' WHERE fld_id='" . $cnt . "'");
                } else {
                    $ObjDB->NonQuery("INSERT INTO itc_mis_points_master SET fld_teacher_points_earned=$mispointsqlfragment, fld_points_possible='" . $mispossible[$i] . "', fld_lock='1', fld_updated_by='" . $uid . "', fld_updated_date='" . date("Y-m-d H:i:s") . "' WHERE fld_id='" . $cnt . "'");
                }
            }
        }
	 	/*********Participation code End here*******/
		...
		//When a blank score is entered for mistestpoint, put NULL into the database
        ...
        for($j=0;$j<sizeof($mistesttypeid);$j++)
		{
            ...
            if($mistesttypeid[$j]!='')
			{
                ...
                //when mistestpoint is blank or '', put a null in the database
                $mistestpointsqlfragment = "'" . intval($mistestpoint[$j]) . "'";
                if ($mistestpoint[$j] == ''){
                    $mistestpointsqlfragment = " NULL ";
                }

                if($cnt!='')
                {
                    $ObjDB->NonQuery("UPDATE itc_mis_points_master SET fld_teacher_points_earned=$mistestpointsqlfragment, fld_points_possible='".$mistestpossible[$j]."', fld_lock='1', fld_updated_by='".$uid."', fld_updated_date='".date("Y-m-d H:i:s")."' WHERE fld_id='".$cnt."'");
                }
                else
                {
                    $ObjDB->NonQuery("INSERT INTO itc_mis_points_master (fld_schedule_type, fld_schedule_id, fld_student_id, fld_mis_id, fld_teacher_points_earned, fld_points_possible, fld_lock, fld_mistype, fld_res_id, fld_grade, fld_created_by, fld_created_date) VALUES('20', '".$scheduleid."', '".$studentid."', '".$unitmodid."', ".$mistestpointsqlfragment.", '".$mistestpossible[$j]."', '1', '".$mistesttype[$j]."', '".$mistesttypeid[$j]."', '1', '".$uid."', '".date("Y-m-d H:i:s")."')");
                }
			}
		}
		/*****Teacher Points earned code End here for pre/post test*****/
            ...
            <tbody>        
                ...
                if($qrystudent->num_rows>0)
                {
                    ...
                    while($rowqrystudent = $qrystudent->fetch_assoc()) // show the module based on number of copies
                    {
                        ...
                        $points_earned_mysqli_result = $ObjDB->QueryObject("SELECT (CASE WHEN fld_lock='0' THEN fld_points_earned WHEN 
                                                                            fld_lock='1' THEN fld_teacher_points_earned END) as points_earned 
                                                                            FROM itc_module_points_master 
                                                                            WHERE fld_schedule_type='" . $scheduletype . "' AND fld_schedule_id='" . $scheduleid . "' 
                                                                            AND fld_module_id='" . $customid . "' AND fld_student_id='" . $studentid . "' 
                                                                            AND fld_session_id='0' AND fld_type='0' AND fld_preassment_id='0' AND fld_delstatus='0'");
                        $single_row = null;
                        if ($points_earned_mysqli_result !== null){
                            $single_row = $points_earned_mysqli_result->fetch_assoc();
                        }

                        if ($single_row == null){
                            $qrypointsearn = null;
                        }else{
                            $qrypointsearn = $single_row['points_earned'];
                        }
