...
$html = file_get_contents(__HOSTADDR__.'reports/pdf-genrate-code/assessmentquestion-output.php?id='.$id.'');
...
class MYPDF extends TCPDF {
	public function Header() { // Page header
		...
		$qryclass = $ObjDB->QueryObject("SELECT a.fld_class_name AS classname, a.fld_period AS period, b.fld_test_name AS assignmentname, CONCAT(c.fld_fname,'',c.fld_lname) AS username 
										FROM itc_class_master AS a 
										JOIN itc_test_master AS b 
										JOIN itc_user_master AS c 
										WHERE a.fld_id='".$ids[1]."' AND b.fld_id='".$ids[3]."' AND c.fld_id='".$ids[2]."' AND b.fld_ass_type='0'");
		...
		$this->Text(10, 18, 'Individual Assessment Question Report');
		...
		$this->Text(10, 55, 'Student : '.$username);
		$alen = strlen ('Assignment : '.$assignmentname);
		
		if($alen>40)
			$alength = 100;
		else
			$alength = 120;
		$this->Text($alength, 50, 'Assignment : '.$assignmentname);
	}
...
$pdf->SetTitle('Individual Assessment Question Report');
