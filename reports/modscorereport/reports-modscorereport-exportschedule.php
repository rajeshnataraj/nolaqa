...
$id = explode(",",$id);
$module = isset($method['assignmentlist']) ? $method['assignmentlist'] : '0';
$modid=explode(",",$module);
...
for($n=0;$n<sizeof($modid);$n++)
{
    ...
    if($scheduletype==6)
    {
        $modulename=$ObjDB->SelectSingleValue("SELECT CONCAT(a.fld_mathmodule_name,' ',b.fld_version)
											...
											WHERE a.fld_id='".$newmodid."' AND b.fld_delstatus='0' AND a.fld_delstatus='0'");
    }
    else
    {
        $modulename=$ObjDB->SelectSingleValue("SELECT CONCAT(a.fld_module_name,' ',b.fld_version)
			                              FROM itc_module_master AS a 
										  LEFT JOIN itc_module_version_track AS b ON b.fld_mod_id='".$newmodid."'    
										  WHERE a.fld_id='".$newmodid."' AND b.fld_delstatus='0' AND a.fld_delstatus='0'");
    }
...
{ 
	...
	for ($x = 0; $x <=2; $x++) {
	     $col = 0;
		if($autoinc == 1 AND $row==1) {
		    $value=$modulename;
		    $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(5, 0, $value);
		    ...
		    $objPHPExcel->getActiveSheet()->getRowDimension($row)->setRowHeight(25);
                ...
                elseif($autoinc==1 AND $row!=1) {
			
				$value=$modulename;
                                $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, $value);
