...
<?php
	...
	include(__DIR__ . "/users-individuals-common-elements.php");
	...
	$qrytag = $ObjDB->QueryObject("SELECT a.fld_id AS tagid,a.fld_tag_name AS tagname 
								...
								WHERE a.fld_id=b.fld_tag_id AND b.fld_tag_type='8' AND b.fld_access='1' AND a.fld_created_by='".$uid."' 
	...
	if($editid != 0){
		...
		if($tdistid == 0 and $tshlid !=0){
		...
		else if($tdistid == 0 and $tshlid ==0 and $tuserid !=0){
			
			$homename = $ObjDB->SelectSingleValue("SELECT CONCAT(fld_fname,' ',fld_lname) 
													FROM itc_user_master 
													WHERE fld_id='".$tuserid."'");
			$arrfieldid=array();
			$arrfieldvalue=array();
			
			$optionaldet1 = $ObjDB->QueryObject("SELECT fld_field_id,fld_field_value 
												FROM itc_user_add_info 
												WHERE fld_user_id='".$tuserid."'");
			$rows1=$optionaldet1->num_rows;
				while($rowoptionaldet1=$optionaldet1->fetch_object())
				{
				 ...
				 $state = $arrcombine1[8];
				 $hstatename = $ObjDB->SelectSingleValue("SELECT DISTINCT(fld_statename) 
				 										FROM itc_state_city 
														WHERE fld_statevalue='".$arrcombine1[8]."'");
				 $city = $arrcombine1[9];
				 
			$selectdet = $ObjDB->QueryObject("SELECT fld_username AS username,fld_fname AS fname,fld_lname AS lname, fld_email AS email,fld_profile_pic AS pphoto 
											FROM itc_user_master 
											WHERE fld_id='".$editid."' ");
		}
		else{
	...
	if($sessmasterprfid == 5 and $editid == 0){
	...
	if(($sessmasterprfid == 7 or $sessmasterprfid == 8)  and $editid == 0){
...
<script type="text/javascript" charset="utf-8">	
	$(function(){				
		var t4 = new $.TextboxList('#form_tags_newteacher', 
		{
...
<section data-type='#users-individuals' id='users-individuals-teacher_newteacher'>
	<div class='container'>
    	<div class='row'>
            <div class='twelve columns'>
            	<p class="dialogTitle"><?php if($editid == 0){ echo "New Teacher";} else { echo $fname." ".$lname." "."Teacher";} ?></p>
            ...
            <div class='eleven columns centered insideForm'>
                <form method='post' name="tadminval" id="tadminval">
                    ...
                    <div class="six columns">
                        <div class="title-info">Teacher Information (Required)</div>
                        Select state<span class="fldreq">*</span>
                        <dl class='field row'>
                            <dt class='dropdown'>
                               <?php if($sessmasterprfid == 5 or ($editid!=0 and $tuserid !=0)){?>
                                    <div class="selectbox">
                                     <input type="hidden" name="ddlstate" id="ddlstate" value="<?php echo $state;?>">
                                    <a class="selectbox-toggle" role="button" data-toggle="selectbox" href="#">
                                    <span class="selectbox-option input-medium" data-option=""><?php echo $hstatename;?></span>
                                    <b class="caret1"></b>
                                    </a>
                                    </div>
                                <?php } 
                            else if(($sessmasterprfid == 7 or $sessmasterprfid ==8)  and $editid == 0){?>
                                <div class="selectbox">
                    ...
                    <div class="six columns">
                        <div class="title-info">Teacher Additional Information (Optional)</div>
                        Street address
                        <dl class='field row'>
                            <dt class='text'>
                ...
                <div class="row rowspacer">
                    ...
                    Select city<span class="fldreq">*</span>
                        <dl class='field row' id="cit">
                            <dt class='dropdown'>
                                <?php if($sessmasterprfid == 5 and $editid == 0){?>
                                    <div class="selectbox">
                                      <input type="hidden" name="ddlcity" id="ddlcity" value="<?php echo $city;?>" >
                                      <a class="selectbox-toggle" role="button" data-toggle="selectbox" href="#">
                                        <span class="selectbox-option input-medium" data-option=""><?php echo $city;?></span>
                                        <b class="caret1"></b>
                                      </a>
                                    </div>
                             <?php } 
                        ...
                        <dl class='field row' id="dit">
                            <dt class='dropdown'>
                                <?php if($sessmasterprfid == 5 or ($editid!=0 and $tuserid !=0)){?>
                                    <div class="selectbox">
                                      <a class="selectbox-toggle" role="button" data-toggle="selectbox" href="#">
                                        <span class="selectbox-option input-medium" data-option=""><?php echo "Individual Purchase";?></span>
                                        <b class="caret1"></b>
                                      </a>
                                  </div>
                                 <?php } 
                                        ...
                                        <ul role="options">
                                            <?php 
                                                $cityqry = $ObjDB->QueryObject("SELECT DISTINCT(fld_cityname) AS cityname 
																				FROM itc_state_city WHERE fld_statevalue='".$arrcombine[8]."' AND fld_delstatus=0 
                        ...
                        <dl class='field row' id="shl">
                            <dt class='dropdown'>
                                <?php if($sessmasterprfid == 5 or ($editid!=0 and $tuserid !=0)){?>
                                    <div class="selectbox">
                                      <a class="selectbox-toggle" role="button" data-toggle="selectbox" href="#">
                                        <span class="selectbox-option input-medium" data-option=""><?php echo $homename;?></span>
                                        <b class="caret1"></b>
                                      </a>
                                  </div>
                             <?php } 
                ...
                <div class="row rowspacer">
                    ...
                    First name<span class="fldreq">*</span>
                        <dl class='field row'>
                            <dt class='text'>
                                <input id="fname" name="fname"  placeholder='First name' tabindex="5" type='text' value="<?php echo $fname;?>"></dt>
                        </dl> 
                    </div>
                    <div class="six columns">
                    Mobile number
                ...
                <?php
                show_username(intval($editid) != 0, $username);
                ?>
                ...
                <div class="row rowspacer">
                    <div class='twelve columns'>
                        ...
                        <div class="tag_well">
                            <input type="text" name="test3" value="" id="form_tags_newteacher" />
                        </div>
                    </div>
                </div>
					...
					<?php 
                        ...
                        <div class="six columns <?php if($status !=1){ echo "dim"; } ?>">
                            <p class='btn secondary twelve columns'>
                              <a tabindex="14" onclick="fn_resett('<?php echo $username;?>')">Reset Password</a>
                            </p>
                        </div>
                     <?php }
                ...
                <div class="row rowspacer">
                    <div class="six columns">
                        <p class='btn primary twelve columns'>
							 <?php if($editid == 0){ ?>
                                 <a tabindex="16" onclick="fn_cancel('users-individuals-teacher');">Cancel</a>
                             <?php } else { ?>
                                <a tabindex="17" onclick="fn_deletteacher(<?php echo $editid;?>)">Delete</a>
                             <?php } ?>   
                        </p>
                    </div>
                    <div class="six columns" id="userphoto">
                        <p class='btn secondary twelve columns'>
                        	<a tabindex="15" onclick="fn_createteacher(<?php echo $editid;?>);"><?php if($editid==0) echo "Create Teacher"; else echo "Update Teacher";?></a>
                        </p>
                    </div>
                </div>
                </form>
