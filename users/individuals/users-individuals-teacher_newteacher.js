...
function fn_changecity(statevalue){
	...
	$.ajax({
		type: 'post',
		url: 'users/individuals/users-individuals-teacher_newteacherdb.php',
		data: dataparam,
		beforeSend: function(){
...
/*----
    fn_createteacher()
	Function to save the teacher detaila
----*/
function fn_createteacher(id){
...
//        New Line for training teacher or not End
	...
	if(id==0){	
		actionmsg = "Saving";
		alertmsg = "Teacher account is created successfully";
	}
	else{
		actionmsg = "Updating";
		alertmsg = "Teacher account is updated successfully";
	}
	if($("#tadminval").validate().form())
	{
		var dataparam = "oper=saveteacher&state="+state+"&city="+city+"&distid="+distid+"&shlid="+shlid+"&fname="+escapestr(fname)+"&lname="+escapestr(lname)+"&address1="+escapestr(address1)+"&email="+email+"&state1="+state1+"&city1="+city1+"&zipcode1="+zipcode1+"&mobileno="+mobileno+"&homeno="+homeno+"&photo="+photo+"&tags="+escapestr($('#form_tags_newteacher').val())+"&id="+id+"&chkteacher="+chkteacher+"&chkitct="+chkitct+"&chksost="+chksost;
		$.ajax({
			type: 'post',
			url: 'users/individuals/users-individuals-teacher_newteacherdb.php',
			data: dataparam,
			...
			success:function(data) {
				if(data=="success"){
					...
					setTimeout('showpages("users-individuals-teacher","users/individuals/users-individuals-teacher.php");',1500);
				}
				else if(data=="fail"){
...
/*----
    fn_deletteacher()
	Function to delet the teacher detaila
----*/
function fn_deletteacher(id){
	
	var dataparam = "oper=deletteacher&editid="+id;
	actionmsg = "Deleting";
	alertmsg ="Teacher account is deleted successfully";
...
function fn_resett(username){

    var dataparam = "oper=resett&username="+username;
...
function fn_resendmail(mailid)
{
	...
	var dataparam = "oper=saveteachermail&mailid="+mailid;
...
function fn_showteachers(schoolid,distid){
    ...
    var dataparam = "oper=showteachers&distid="+distid+"&schoolid="+schoolid;		
    $.ajax({
            type: 'post',
            url: 'users/individuals/users-individuals-teacher_newteacherdb.php',
            data: dataparam,
