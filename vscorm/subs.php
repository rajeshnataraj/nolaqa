...
function readElement($VarName) {
	...
	$result = mysql_query("select VarValue from itc_assignment_lesson_scorm_track where ((SCOInstanceID=$SCOInstanceID) and (VarName='$safeVarName'))",$link);
	//$result = mysql_query("select varValue from itc_assignment_lesson_scorm_track where ((SCOInstanceID=$SCOInstanceID) and (varName='$safeVarName') and (lesson_id='$safeVarlesson') and (student_id='$safeVarstudent'))",$link);
...
function writeElement($VarName,$VarValue) { 
	...
	mysql_query("update itc_assignment_lesson_scorm_track set VarValue='$safeVarValue' where ((SCOInstanceID=$SCOInstanceID) and (VarName='$safeVarName'))",$link);
	//mysql_query("update itc_assignment_lesson_scorm_track set VarValue='$safeVarValue' where ((SCOInstanceID=$SCOInstanceID) and (VarName='$safeVarName') and (lesson_id='$safeVarlesson') and (student_id='$safeVarstudent'))",$link);
...
function initializeElement($VarName,$VarValue) {
	...
	if (! mysql_num_rows($result)) {
		mysql_query("insert into itc_assignment_lesson_scorm_track (SCOInstanceID,VarName,VarValue) values ($SCOInstanceID,'$safeVarName','$safeVarValue')",$link);
	}
...
function initializeSCO() {
	...
	$result = mysql_query("select count(VarName) from itc_assignment_lesson_scorm_track where (SCOInstanceID=$SCOInstanceID)",$link);
	...
	$result = mysql_query("select VarName,VarValue from itc_assignment_lesson_scorm_track where (SCOInstanceID=$SCOInstanceID)",$link);
