<?php
		...
		for($i=0;$i<sizeof($tmpqanswer);$i++){
			...
			if($scheduletype<5) //5
				$qry = $ObjDB->QueryObject("SELECT fld_points AS pointspossible, fld_grade AS grade 
											...
											WHERE fld_session_id='".$sessionid."' AND fld_flag='1' AND fld_module_id='".$tempmoduleid."'");
			else
				$qry = $ObjDB->QueryObject("SELECT fld_points AS pointspossible, fld_grade AS grade 
											FROM itc_module_wca_grade 
											WHERE fld_session_id='".$sessionid."' AND fld_flag='1' AND fld_module_id='".$moduleid."' 
												AND fld_schedule_id='".$scheduleid."' AND fld_preassment_id='".$pageid."' AND fld_type='0'");
	...
	if($oper=="readpages" and $oper != "" ){
		...
		if($scheduletype!=2 and $scheduletype!=3 and $scheduletype!=8)
		{
			if($scheduletype==1 or $scheduletype==4)
			{
				$tablename = "itc_class_rotation_extcontent_mapping";
				if($scheduletype==1)
					$maptablename = "itc_extendtext_master_mapping_text";
				if($scheduletype==4)
					$maptablename = "itc_extendtextmath_master_mapping_text";
			}
				
			else if($scheduletype==5 or $scheduletype==6 or $scheduletype==7)
			{
				$tablename = "itc_class_indassesment_extcontent_mapping";
				if($scheduletype==7)
					$maptablename = "itc_extendtextquest_master_mapping_text";
				if($scheduletype==5)
					$maptablename = "itc_extendtext_master_mapping_text";
				if($scheduletype==6)
					$maptablename = "itc_extendtextmath_master_mapping_text";
			...
			$extid = $ObjDB->SelectSingleValueInt("SELECT fld_ext_id FROM ".$tablename." WHERE fld_schedule_id='".$scheduleid."' AND fld_module_id='".$moduleid."' AND fld_active='1'");
		}
		else
			$extid = 0;
		...
		$pagesreadstatus = $ObjDB->QueryObject("SELECT IFNULL(GROUP_CONCAT(CASE WHEN fld_tester_id='".$uid."' THEN fld_page_id END),'-') AS pageids, 
														IFNULL(GROUP_CONCAT(CASE WHEN fld_tester_id='".$uid1."' THEN fld_page_id END),'-') AS pageids1
												FROM itc_module_play_track 
												WHERE fld_section_id='".$sessionid."' AND fld_module_id='".$moduleid."' AND fld_schedule_id='".$scheduleid."' 
		...
		if($pagesreadstatus->num_rows > 0){
			...
			$tmppgid1 = explode(",", $pageids1);
			
			if(max($tmppgid) >= max($tmppgid1))			
				$tmpgreater = max($tmppgid);
			else
				$tmpgreater = max($tmppgid1);
			
			for($i=0;$i<=$tmpgreater;$i++){
				if($i==0)
				{
					if(min($tmppgid) ==0 and min($tmppgid1)==0 and min($tmppgid) !='-' and min($tmppgid1)!='-')
						$readstatus[$i] = 1;
					else
						$readstatus[$i] = 0;
				}
		...
		if($extid!=0 and $extid!='')
		{
			$extend = $ObjDB->QueryObject("SELECT IFNULL(GROUP_CONCAT(fld_page_id SEPARATOR '~~'),0) AS pagids, GROUP_CONCAT(fld_extend_content_text SEPARATOR '~~') AS extcontent
											FROM ".$maptablename."
											WHERE fld_section_id='".$sessionid."' AND fld_module_id='".$moduleid."' AND fld_extend_id='".$extid."' AND fld_delstatus='0'");
													
			if($extend->num_rows > 0){
				$rowextend = $extend->fetch_assoc();
				extract($rowextend);
				
				$tmppg = explode("~~", $pagids);
				$tmpext = explode("~~", $extcontent);
				
				for($i=0;$i<sizeof($tmppg);$i++){
					$extentstatus[$tmppg[$i]] = 1;	
					$extentcontent[$tmppg[$i]] = $tmpext[$i];		
				}
			}
		}
		
		echo implode(",",$readstatus)."!~".implode(",",$extentstatus)."!~".implode("~~",$extentcontent);			
	}
	...
	if($oper=="showatten" and $oper != " " )
	{
		...
		$atten = $ObjDB->SelectSingleValue("SELECT (CASE WHEN fld_lock='1' THEN fld_teacher_points_earned WHEN fld_lock='0' THEN fld_points_earned END) AS points 
											FROM itc_module_points_master 
		...
		$partic = $ObjDB->SelectSingleValue("SELECT (CASE WHEN fld_lock='1' THEN fld_teacher_points_earned WHEN fld_lock='0' THEN fld_points_earned END) AS points 
											FROM itc_module_points_master 
											WHERE fld_module_id='".$moduleid."'  AND fld_schedule_id='".$scheduleid."' AND fld_schedule_type='".$scheduletype."' 
		...
		if($testerid1 != '') {
			...
			$atten1 = $ObjDB->SelectSingleValue("SELECT (CASE WHEN fld_lock='1' THEN fld_teacher_points_earned WHEN fld_lock='0' THEN fld_points_earned END) AS points 
												FROM itc_module_points_master 
												WHERE fld_module_id='".$moduleid."'  AND fld_schedule_id='".$scheduleid."' AND fld_schedule_type='".$scheduletype."' 
													AND fld_session_id='".$sessionid."' AND fld_type='1' AND fld_student_id='".$testerid1."' AND fld_delstatus='0'");
		
			$partic1 = $ObjDB->SelectSingleValue("SELECT (CASE WHEN fld_lock='1' THEN fld_teacher_points_earned WHEN fld_lock='0' THEN fld_points_earned END) AS points 
												FROM itc_module_points_master 
												WHERE fld_module_id='".$moduleid."'  AND fld_schedule_id='".$scheduleid."' AND fld_schedule_type='".$scheduletype."' 
