...
gantt.attachEvent("onLightbox", function (task_id){
    //alert("hai"+task_id);
        var strid = task_id.indexOf("~");//Find the symbol '~' in task_id
        if(strid!='-1')
        {
            fn_facebox(task_id);
        }
...
function fn_facebox(taskid)
{
    ...
    $.ajax({
            type	: "POST",
            cache	: false,
            url		: "stdprogressajax.php",
            data		: "oper=showassignments&taskid="+taskid,
            success: function(data) {
                    $.fancybox(data);
                    $('#fancybox-wrap').css("width","600px");
                    $('#fancybox-content').css("width","580px");
                    $('#fancybox-content').css("background","#D0E0EB");
            }
    });
}

function getType(task_id) {
    var m = (/[\d]+(\.[\d]+)?/).exec(task_id);
    if (m) {
...
function fn_destuncheck(undest,studentid,schudleid,expid,expormis,schtype){
    var dataparam = "oper=uncheckdest&destid="+undest+"&studentid="+studentid+"&schudleid="+schudleid+"&expid="+expid+"&expormis="+expormis+"&schtype="+schtype;
     alert(dataparam);
...
function fn_taskuncheck(untask,studentid,schudleid,expid,expormis,schtype){
     //alert(untask);
     var dataparam = "oper=taskuncheck&taskid="+untask+"&studentid="+studentid+"&schudleid="+schudleid+"&expid="+expid+"&expormis="+expormis+"&schtype="+schtype;
      alert(dataparam);
...
function fn_resuncheck(unres,studentid,schudleid,expid,expormis,schtype){
     var dataparam = "oper=resuncheck&resid="+unres+"&studentid="+studentid+"&schudleid="+schudleid+"&expid="+expid+"&expormis="+expormis+"&schtype="+schtype;
     alert(dataparam);
