<?php
	...
	else
	{
		...
		$schename="";
	        ...
	        $startrotation=$ObjDB->SelectSingleValueInt("SELECT max(a.fld_rotation) FROM itc_class_rotation_expschedulegriddet AS a 
												LEFT JOIN itc_exp_dest_play_track AS b ON a.fld_expedition_id=b.fld_exp_id AND a.fld_student_id=b.fld_student_id 
												WHERE a.fld_schedule_id='".$scheduleid."' AND b.fld_schedule_type='19' AND b.fld_schedule_id='".$scheduleid."' AND a.fld_flag='1' AND b.fld_delstatus='0'");
		...
		$qrymodule=$ObjDB->QueryObject("SELECT fld_exp_id as expid,'17' as type,fld_numberofrotation as numberofrotations FROM itc_class_rotation_expmoduledet WHERE fld_class_id='".$classid."' AND fld_schedule_id='".$scheduleid."' AND fld_flag=1 order by fld_row_id ASC");
		
		$numberofrotations=$ObjDB->SelectSingleValueInt("SELECT fld_numberofrotation FROM itc_class_rotation_expmoduledet WHERE fld_class_id='".$classid."' AND fld_schedule_id='".$scheduleid."' AND fld_flag=1");
		
		$qrygetscheduledet=$ObjDB->QueryObject("SELECT fld_generatetype,fld_autoblock,fld_flag,fld_numberofcopies as numberofcopies,fld_startdate as startdate,fld_rotationlength as rotationlength FROM itc_class_rotation_expschedule_mastertemp WHERE fld_id='".$scheduleid."'");
...
<section data-type='#class-newclass' id='class-newclass-viewschedule_editexp'>
                    	...
                    	<div class="row rowrotation"> <!-- Start and End Rotation textbox -->
                            ...
                            <div class="two columns">
                              <div style="margin-left:30px;">
                                ...
                                <input type="checkbox" name="autoblock" id="autoblock" onclick="fn_autoblockexp(<?php echo $scheduleid;?>);" <?php if($fld_autoblock==1){?> checked="checked" <?php } ?> />
                              </div>
                            ...
                            <div class="six columns <?php if($id[3]=="viewrot"){?> dim <?php } ?>">
                                ...
                                &nbsp;&nbsp;<input type="button" style="cursor:pointer;width:120px;text-align:center;" id="addmodinc" class="addmodinc btn" value="Add Exp" onclick="fn_showexpedition(<?php echo $scheduleid;?>);" />
                                ...
                                &nbsp;&nbsp;&nbsp;&nbsp;<input type="button" style="cursor:pointer;width:100px;height:50px;text-align:center;" id="save" onclick="fn_saveexpdetails();" class="save btn" value="Save"/>
                            </div>
...
?>
	...
	<script>
        ...
        <?php 
                ...
                {
                ?>
                    fn_autoblockexp(<?php echo $scheduleid;?>);
               <?php
                }
                ?>
    	$('#stuidname').val(stuname);
			...
			{
				 <?php
				$qrycelldet=$ObjDB->QueryObject("SELECT CONCAT(b.fld_lname,' ',b.fld_fname) AS studentname,a.fld_cell_id AS cellid,a.fld_student_id AS studentid  FROM itc_class_rotation_expschedulegriddet AS a 
				            LEFT JOIN itc_user_master AS b ON b.fld_id=a.fld_student_id  
							WHERE a.fld_class_id='".$classid."' AND a.fld_schedule_id='".$scheduleid."' AND a.fld_flag=1 AND b.fld_delstatus='0'");
...
$qrystudent=$ObjDB->QueryObject("SELECT a.fld_studentid AS stuid, a.fld_expeditionid AS modid,'17' as type
                                        FROM itc_class_rotation_expblockstudent AS a
                                        LEFT JOIN itc_class_rotation_expmoduledet AS b ON b.fld_exp_id=a.fld_expeditionid
