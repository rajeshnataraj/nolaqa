<?php
	...
	else
	{
		...
		$schename="";
	        ...
	        $startrotation=$ObjDB->SelectSingleValueInt("SELECT max(a.fld_rotation) FROM itc_class_rotation_schedulegriddet AS a 
												LEFT JOIN itc_module_play_track AS b ON a.fld_module_id=b.fld_module_id AND a.fld_student_id=b.fld_tester_id 
												WHERE a.fld_schedule_id='".$scheduleid."' AND b.fld_schedule_id='".$scheduleid."' AND a.fld_flag='1' AND b.fld_delstatus='0'");
		if($startrotation=='')
		{
			$startrotation=1;
		}
	
		$qrymodule=$ObjDB->QueryObject("SELECT fld_module_id as moduleid,fld_type as type,fld_module_name as modulename,fld_numberofrotation as numberofrotations FROM itc_class_rotation_moduledet WHERE fld_class_id='".$classid."' AND fld_schedule_id='".$scheduleid."' AND fld_flag=1 order by fld_row_id ASC");
		
		$numberofrotations=$ObjDB->SelectSingleValueInt("SELECT fld_numberofrotation FROM itc_class_rotation_moduledet WHERE fld_class_id='".$classid."' AND fld_schedule_id='".$scheduleid."' AND fld_flag=1");
		
		$qrygetscheduledet=$ObjDB->QueryObject("SELECT fld_generatetype,fld_autoblock,fld_moduletype,fld_flag,fld_numberofcopies as numberofcopies,fld_startdate as startdate,fld_rotationlength as rotationlength FROM itc_class_rotation_schedule_mastertemp WHERE fld_id='".$scheduleid."'");
...
<section data-type='#class-newclass' id='class-newclass-viewschedule_edit'>
                    	...
                    	<div class="row rowrotation"> <!-- Start and End Rotation textbox -->
                            ...
                            <div class="two columns">
                              <div style="margin-left:30px;">
                                ...
                                <input type="checkbox" name="autoblock" id="autoblock" onclick="fn_autoblock(<?php echo $scheduleid;?>);" <?php if($fld_autoblock==1){?> checked="checked" <?php } ?> />
                              </div>
                            ...
                            <div class="six columns <?php if($id[3]=="viewrot"){?> dim <?php } ?>">
                            	<input type="button" class="btn" value="Generate" id="generatebtn" name="generate" onclick="fn_schedulegenerate();" />
                                ...
                                &nbsp;&nbsp;<input type="button" style="cursor:pointer;width:120px;text-align:center;" id="addmodinc" class="addmodinc btn" value="Add Module" onclick="fn_showmodule(<?php echo $scheduleid;?>);" />
                                ...
                                &nbsp;&nbsp;&nbsp;&nbsp;<input type="button" style="cursor:pointer;width:100px;height:50px;text-align:center;" id="save" onclick="fn_save();" class="save btn" value="Save"/>
                            </div>
											...
											while($rowmodule = $qrymodule->fetch_assoc()) // show the module based on number of copies
											{
												extract($rowmodule);
													...
													<tr id="tr_<?php echo $i;?>" class="<?php echo $moduleid."-".$type;?>">
														<td id="module_<?php echo $i;?>" style="cursor:default;" <?php if(isset($id[3])!="viewrot"){ ?> onmouseover="fn_checkcellvalue(<?php echo $i;$i++;?>)" <?php } ?>><?php 
			if($fld_moduletype==1)
			{
				if($type==1)
				{
				...
				{
					echo $ObjDB->SelectSingleValue("SELECT fld_contentname from itc_customcontent_master WHERE fld_id='".$moduleid."' AND fld_delstatus='0'");
				}
			}
			else
			{
				if($type==2)
				{
					echo $ObjDB->SelectSingleValue("SELECT CONCAT(a.fld_mathmodule_name,' ',b.fld_version)
				...
				else
				{
					echo $ObjDB->SelectSingleValue("SELECT fld_contentname from itc_customcontent_master WHERE fld_id='".$moduleid."' AND fld_delstatus='0'");
                        ...
                        <input type="hidden" name="moduletype" id="moduletype" value="<?php echo $fld_moduletype; ?>" />
...
?>
	...
	<script>
        ...
        <?php 
                ...
                {
                ?>
                    fn_autoblock(<?php echo $scheduleid;?>);
               <?php
                }
                ?>
    	$('#stuidname').val(stuname);
			...
			{
				 <?php
				$qrycelldet=$ObjDB->QueryObject("SELECT CONCAT(b.fld_lname,' ',b.fld_fname) AS studentname,a.fld_cell_id AS cellid,a.fld_student_id AS studentid                             FROM itc_class_rotation_schedulegriddet AS a 
				            LEFT JOIN itc_user_master AS b ON b.fld_id=a.fld_student_id  
							WHERE a.fld_class_id='".$classid."' AND a.fld_schedule_id='".$scheduleid."' AND a.fld_flag=1 AND b.fld_delstatus='0'");
...
$qrystudent=$ObjDB->QueryObject("SELECT a.fld_studentid AS stuid, a.fld_moduleid AS modid,b.fld_type as type
                                        FROM itc_class_rotation_blockstudent AS a
                                        LEFT JOIN itc_class_rotation_moduledet AS b ON b.fld_module_id=a.fld_moduleid
    ...
    if($qrystudent->num_rows>0)
    {
        while($rowstudent=$qrystudent->fetch_assoc())
        {
           extract($rowstudent);
            ...
            $studblock[]=$modid."-".$type; 
            $stumodid[]=$modid."-".$type."-".$stuid;
        }
    } 
    ?>
    <input type="hidden" name="blockmod" id="blockmod" value=<?php echo json_encode($studblock);?> />
