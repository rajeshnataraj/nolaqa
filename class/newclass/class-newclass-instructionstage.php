...
$qry=$ObjDB->NonQuery("SELECT fld_schedule_id,fld_stagename,fld_startdate,fld_enddate,fld_orientationmod,fld_numberofrotation,fld_adjacentflag from itc_class_dyad_schedule_insstagemap where fld_id='".$id[2]."' and fld_flag=1");
...
{
		$prevendate=$ObjDB->SelectSingleValue("SELECT  fld_enddate FROM itc_class_dyad_schedule_insstagemap WHERE fld_id<'".$id[2]."' AND fld_schedule_id='".$fld_schedule_id."' AND fld_flag='1' AND fld_enddate<>'0000-00-00' ORDER BY fld_id DESC LIMIT 0,1");	
...
?>
<section data-type='#class-newclass' id='class-newclass-instructionstage'>
	<div class='container'>
        <div class='row'>
            <div class="span10">
                    ...
                    <div class='eleven columns centered insideForm'>
                    	<form id="stageform" name="stageform">
                        	...
                        	<div class='row rowspacer'>
                                ...
                                Select Stage Value<span class="fldreq">*</span>
                                    <dl class='field row'>   
                                        <dt class='dropdown'>   
                                            <div class="selectbox">
                                                <input type="hidden" name="stagevalue" id="stagevalue" value="<?php echo $stageid;?>"  onchange="fn_loadstagetype()" />
                                ...
                                Select Stage Type<span class="fldreq">*</span>
                                    <dl class='field row'>   
                                        <dt class='dropdown'>   
                                            <div class="selectbox" id="dyadstagetype">
                         ...
                         <div class='row rowspacer orientationdd' <?php if($id[1]==1 or $id[1]==3){?>style="display:none;<?php }?>">
                            ...
                            <div class='row rowspacer rotationdd' <?php if($id[1]!=3){?>style="display:none;<?php }?>">
                                ...
                                Select Rotation<span class="fldreq">*</span>
                                    <dl class='field row'>   
                                        <dt class='dropdown'>   
                                            <div class="selectbox">
                                                <input type="hidden" name="rotation" id="rotation" value="<?php if($fld_numberofrotation!='0'){echo $fld_numberofrotation;}?>" onchange="fn_setenddate();"/>
                                                <a class="selectbox-toggle" role="button" data-toggle="selectbox" href="#">
                            ...
                            <div id="loaddyadmodule"></div>
                            <input type="hidden" name="instype" id="instype" value="<?php echo $instype;?>" />
                            <input type="hidden" name="insstageid" id="insstageid" value="<?php echo $id[2];?>" />
                            
                            <div class="row">
                                <div class="tRight">
                                    <!--<input type="button" id="btnstep" class="darkButton" style="width:200px; height:42px;float:right;" value="Save Instruction" onClick="fn_savedyadinsschedule('ins',0,'save');" />-->
                                    <input type="button" id="btnstep" class="darkButton" style="width:200px; height:42px;float:right;" value="Save Instruction" onClick="fn_checkdyadstagedate();" />
                                </div>
                            </div>
		...
		$(function(){
									$("#stageform").validate({
										...
										messages: { 
											...
											distartdate:{  required: "select start date" },
											dienddate:{ required: "Select end date", greaterThan: "Enddate must be greater"},
