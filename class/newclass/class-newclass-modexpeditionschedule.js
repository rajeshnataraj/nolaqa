...
function fn_modexploadcontent(lid,sid,type)
{
	        if(type==0)
                {
                   sid=$('#modexpschtemplateid').val();
                }
		dataparam="oper=modexploadcontent&lid="+lid+"&sid="+sid+"&classid="+$('#hidclassid').val()+"&type="+type;	
		$.ajax({
			type: 'post',
			...
			success:function(ajaxdata) {
				...
				if(sid!=0)
				{
					...
					{
					setTimeout('showpageswithpostmethod("class-newclass-viewschedule_editmodexp","class/newclass/class-newclass-viewschedule_editmodexp.php","id='+sid+","+classid+'");',500);	
					}
...
function fn_loadmodexpmodules(scheduleid,type,asstype)
{
	var lid = $('#licenseid').val();		
		...
		$.ajax({
			...
			success:function(ajaxdata) {
				...
				$('#modnxtstep').show();
				fn_loadexpmodexpeditions(scheduleid,type,asstype);
                      }
		});
} 

/* get Expeditions based on license id */
function fn_loadexpmodexpeditions(scheduleid,type,asstype)
{
	var lid = $('#licenseid').val();		
		...
		$.ajax({
			...
			success:function(ajaxdata) {
				...
				$('#modnxtstep').show();
                                ...
                                setTimeout("fn_modexpblockstudent();",1000);
                                
				if(scheduleid>0)
				{					
                                        fn_showinlineass();
					fn_rotloadexpmateriallist(scheduleid,lid,"exc");
					fn_rotloadexpeditionextendcontent(scheduleid,lid,"exc"); // created by chandru load expedition schedule list
                                        fn_rotloadmoduleextendcontent(scheduleid,lid,"exc");
				}
                                else
                                {
                                    fn_modexpchecking();
...
function fn_modexpchecking()
{
	...
	if($('#schtype').val()=="edit")
        ...
        {
	      var modcount=list4.length+parseInt(list2.length);
        }
...
function fn_modexpblockstudent()
{
    ...
    dataparam="oper=blockstudents&licenseid="+lid+"&scheduleid="+$('#scheduleid').val()+"&classid="+$('#hidclassid').val()+"&students="+list10+"&studenttype="+$('#studenttype').val()+"&blockmodexpedition="+$('#selectblockmodexpedition').val();	
    ...
    $.ajax({
			...
			data: dataparam,		
			success:function(ajaxdata) {
				$('#blockmodexpstudent').html(ajaxdata);
                        }
		});
}

/* Get Blocked Expeditions */
function fn_blockmodexpeditions()
{
    var lid = $('#licenseid').val();
    var list4 = [];
    var list2 = [];
    ...
    var dataparam="oper=blockmodexpeditions&licenseid="+lid+"&scheduleid="+$('#scheduleid').val()+"&expeditions="+list2+"&modules="+list4;	
    ...
    $.ajax({
			...
			data: dataparam,		
			success:function(ajaxdata) {
				$('#blockmodexpeditions').html(ajaxdata);
                        }
		});
}
...
function fn_rotloadexpmateriallist(scheduleid,licenseid,type)
{
	...
	var dataparam = "oper=loadmateriallist&list2="+list2+"&scheduleid="+scheduleid+"&licenseid="+licenseid;	
...
function fn_rotloadexpeditionextendcontent(scheduleid,licenseid,type)
{
	...
	var dataparam = "oper=loadexpextendcontent1&list2="+list2+"&scheduleid="+scheduleid+"&licenseid="+licenseid;	
...
function fn_rotloadmoduleextendcontent(scheduleid,licenseid,type)
{
	...
	var dataparam = "oper=loadmodextendcontent&list4="+list4+"&scheduleid="+scheduleid+"&licenseid="+licenseid;	
...
function fn_blockmodexpstudent()
{
    ...
    dataparam="oper=blockmodstudents&scheduleid="+$('#scheduleid').val()+"&classid="+$('#hidclassid').val()+"&students="+list26+"&blockmodule="+$('#selectblockmodexpedition').val();	
    ...
    $.ajax({
            ...
            {
                fn_savemodexpeditionalschedule(2);
            }
}


/* save the Expedition schedular details */
function fn_savemodexpeditionalschedule(flag)
{
	...
	if($("#scheduleform").validate().form() && $("#sform").validate().form())
	{
                ...
                $("div[id^=list2_]").each(function()
		{
			...
			$("input[id^=exp_]").each(function()
			{
					var mlessonid=$(this).val();
					var mlessonid=$(this).attr('name').replace('exp_','');					
					if($('#exp_'+mlessonid).is(':checked')){
	   ...
	   /***********Chandru Updated by [18-12-2015] one or more Extend Content option code end here*********/
		...
		if(list4=='' && list2=='')
		{
		 $.Zebra_Dialog('<strong>Please select anyone content Module or Expedition</strong>', {
		'buttons':  false,
		'auto_close': 3000
		});
		return false;
		...
		$("input[id^=exidmt_]").each(function()
		{
		...
		var dataparam="oper=saveschedule&sname="+sname+"&startdate="+$('#startdate').val()+"&scheduletype="+stype+"&studenttype="+$('#studenttype').val()+"&numberofcopies="+$('#numberofcopies').val()+"&numberofrotations="+$('#numberofrotations').val()+"&rotationlength="+$('#rotationlength').val()+"&sid="+$('#hidscheduleid').val()+"&students="+list10+"&expeditions="+list2+"&modules="+list4+"&classid="+$('#hidclassid').val()+"&licenseid="+$('#licenseid').val()+"&unstudents="+list9+"&extids="+extids+"&blockmodule="+$('#selectblockmodexpedition').val()+"&blockstudents="+list26+"&expids="+expids+"&selectallexpids="+selectallexpids+"&modids="+modids+"&selectallmodids="+selectallmodids+"&selectchkboxids="+selectchkboxids+"&exptest="+exptest+"&desttest="+desttest+"&tasktest="+tasktest+"&restest="+restest; //add last two values by chandru
                //alert(dataparam);
			...
			success:function(data) {
				...
				if(trim(data[0])=="success")
				{
                                        if(flag==0)
					{
						...
						setTimeout('showpageswithpostmethod("class-newclass-viewschedule_createmodexp","class/newclass/class-newclass-viewschedule_createmodexp.php","id='+sid+","+classid+'");',500);
						}
						else
						{
					...
					else if(flag==1)
					{
						...
						setTimeout('showpageswithpostmethod("class-newclass-viewschedule_editmodexp","class/newclass/class-newclass-viewschedule_editmodexp.php","id='+sid+","+stype+","+classid+","+sname+',viewrot'+'");',1000);
...
function fn_autoblockmodexp(sid)
{
...
function fn_modexpgenerate(rotflag)
{
...
function fn_savemodexpdetails()
{
	...
	else
	{
        ...
        var dataparam="oper=saverotation&moduledet="+module+"&numberofrotation="+numberofrotation+"&celldet="+cell+"&classid="+$('#hidclassid').val()+"&scheduleid="+$('#scheduleid').val()+"&autoblock="+autoblock+"&startdate="+$('#start_date').val()+"&rotlength="+$('#rotationlength').val()+"&generatetype="+generatetype+"&blockmodule="+$('#selectblockmodexpedition').val()+"&blockstudents="+list26;
	...
	$.ajax({
		...
		success:function(data) {			
			...
			else
			{
				...
				showloadingalert("Saved Sucessfully.");	
				setTimeout("closeloadingalert();",1000);
				fn_savemodexpeditionalschedule(1);
			}
		}
	});
    }
...
function fn_showmodexpedition(scheduleid)
{	
	...
	$.fancybox.showActivity();
		...
		$.ajax({
			...
			cache	: false,
			url		: "class/newclass/class-newclass-modexpedition-ajax.php",
			data		: "oper=showmodexpedition&scheduleid="+scheduleid+"&licenseid="+licenseid,
			success: function(data) {
				$.fancybox(data);
			}
		});
...
function fn_addmodexpedition(expid,scheduleid,type)
{
	$.fancybox.close();
        var numberofrotation=parseInt($("#myTable05 th").length)-parseInt(1);
	var dataparam = "oper=addmodexpedition&modexpid="+expid+"&trlength="+$("#myTable05 tr:last").prev().attr('id')+"&thlength="+$("#myTable05 tr:first th").length+"&scheduleid="+scheduleid+"&scheduletype="+$('#scheduletype').val()+"&type="+type+"&classid="+$('#hidclassid').val()+"&numberofrotation="+numberofrotation+"&mode="+$('#schtype').val();
	...
	$.ajax({
			...
			success:function(data) {
				...
				setTimeout("closeloadingalert();",1000);
                                ...
                                {
                                    showloadingalert("Custom Content added to table.");
                                }
                                else if(type==2)
                                {
