...
function fn_showphses(unitid){
var dataparam = "oper=showphases"+"&unitid="+unitid;
        $.ajax({
			type: 'post',
			url: 'library/library-scienceofspeed-showphases.php',
			data: dataparam,		
			success:function() {		
		...
		$.ajax({
			...
			success:function(data) {		
				if(data=="success")
				{
					...
					setTimeout('closeloadingalert()',1000);
					setTimeout('removesections("#library-phase");',500);
				...
				$.ajax({
					...
					success:function(data) {		
						if(data=="success")
						{
							...
							setTimeout('closeloadingalert()',1000);
							setTimeout('removesections("#library-phase");',500);
							setTimeout('showpages("library-phase","library/pahse/library-phase.php");',500);
						}
						else
						{
