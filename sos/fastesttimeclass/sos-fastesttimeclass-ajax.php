...
{
	$sosclsid= isset($method['sosclsid']) ? $method['sosclsid'] : '0';
       ...
       //----- New Query next and previews button start page-----// 
        ...
        for($j=0;$j<=sizeof($studentids);$j++)
        {
            ...
            $racetime = $ObjDB->SelectSingleValue("SELECT b.fld_datasheet_recordname AS dsrecordname
                                                        ...
                                                        WHERE  b.fld_datasheet_id='".$sheetid."' AND b.fld_view_cellid='".$racetimeid."' AND b.fld_delstatus='0' AND a.fld_created_by='".$uid."' ");
...
<style>
    ...
    .darkButton, .darkButtonDisabled {
    background: rgba(0, 0, 0, 0) linear-gradient(180deg, #ebf7f8 0%, #cdcece 93%, #cbcbcb 100%) repeat scroll 0 0;
    border: 1px solid #999;
    border-radius: 3px;
    box-shadow: 0 2px 3px rgba(0, 0, 0, 0.3), 0 0 2px rgba(255, 255, 255, 0.65) inset, 0 -1px 2px rgba(0, 0, 0, 0.3) inset;
    color: #24475f;
    cursor: pointer;
...
<a class="icon-synergy-close-dark tooltip" onclick="fn_cancelextendform()" title="Close" href="javascript:void(0);"></a>
    ...
    $qrycelldet=$ObjDB->QueryObject("SELECT fld_datasheet_id AS dsid, fld_datasheet_recordname AS recordname, fld_view_cellid AS cellid FROM itc_sos_datasheet_records WHERE fld_datasheet_id='".$sheetid."' AND fld_delstatus='0' and fld_created_by='".$uid."' ");
...
if($oper == "nextstudent" and $oper != '') 
{
        ...
        if($status=='1')
        {
            ...
            $qry = $ObjDB->QueryObject("SELECT fld_datasheet_id AS dsid, fld_datasheet_recordname AS recordname, fld_stu_id AS studentid FROM itc_sos_datasheet_records
                                                            WHERE fld_datasheet_id='".$sheetid."' AND fld_stu_id='".$next."' AND fld_delstatus='0' and fld_created_by='".$uid."'  limit 1,16");
        }
        else
        {
            ...
            $previews = $result[$key-1];
            $qry = $ObjDB->QueryObject("SELECT fld_datasheet_id AS dsid, fld_datasheet_recordname AS recordname, fld_stu_id AS studentid FROM itc_sos_datasheet_records
                                                            WHERE fld_datasheet_id='".$sheetid."' AND fld_stu_id='".$previews."' AND fld_delstatus='0' and fld_created_by='".$uid."'  limit 1,16");
        }
        if($qry->num_rows>0)
        {												
